...
{
   vec->setSize( count );
   return sio.read( vec->size() * sizeof(typename T::value_type) /* WASM-PORT */, (char*)vec->address() );   
}
...
{
   return sio.write( vec.size() * sizeof(typename T::value_type) /* WASM-PORT */, (char*)vec.address() );   
}
