...
// WASM-PORT: Resource<T> method bodies moved below the ResourceManager
// class definition (end of file) — clang two-phase lookup rejects the
// non-dependent obj->rm-> accesses while ResourceManager is incomplete.
...
public:
   ...
   // WASM-PORT: register a single VOL file that was fetched over HTTP and written into the MEMFS at
   // <basePath>/<volName> at RUNTIME (after boot), then mount it. Mirrors the file-registration tail
   // of searchPath() for one file so addVolume() can find it WITHOUT re-running setSearchPath (which
   // detaches every already-registered file/volume). Enables on-demand asset streaming.
   bool wasmMountVolume(const char *basePath, const char *volName, int fileSize);
#endif
...
// WASM-PORT: relocated from above ResourceManager's definition (see note there)
template<class T>
