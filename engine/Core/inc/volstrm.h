...
#pragma option -a1
#endif
// WASM-PORT: VolumeItem is read as a raw record array straight from the .vol
// on disk, so its layout MUST be the packed 1-byte format the files were
// authored with. The pack pragma was keyed on _MSC_VER only; clang/emscripten
// doesn't define that, so the struct got default alignment (sizeof 20 vs the
// on-disk 17) and every field misaligned -> garbage string offsets. clang
// honors #pragma pack, so apply it here too.
...
#pragma option -a.
#endif
#if defined(_MSC_VER) || defined(__clang__)   // WASM-PORT: balance the pack(push,1) above
#pragma pack(pop)
#endif
