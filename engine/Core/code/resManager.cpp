...
//------------------------------------------------------------------------------
// WASM-PORT: register a runtime-fetched VOL (already written to MEMFS at basePath/volName) into the
// search dictionary, then mount it. Mirrors the file-registration tail of searchPath() (the
// createResource + pushBehind + File-flag/size lines) for ONE file, so we don't re-run setSearchPath
// (which detaches every file/volume already registered). Used by the on-demand asset streamer.
bool ResourceManager::wasmMountVolume(const char *basePath, const char *volName, int fileSize)
{
   StringTableEntry filePath = stringTable.insert((char*)basePath);
   StringTableEntry fileName = stringTable.insert((char*)volName);
   // virtPath = NULL: top-level file directly in basePath (matches searchPath's basePath case and
   // addVolume's getPaths("foo.vol") -> empty path lookup).
   ResourceObject *ro = createResource(NULL, fileName, filePath, fileName);
   dictionary.pushBehind(ro, ResourceObject::File);
   ro->flags     = ResourceObject::File;
   ro->fileOffset = 0;
   ro->fileSize   = fileSize;
   ro->volume     = NULL;
   return addVolume(volName);
}
#endif
