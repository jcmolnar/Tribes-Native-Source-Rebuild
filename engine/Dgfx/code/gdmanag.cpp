...
GetDeviceManager(HWND appWind)
{
   if(!referenceCount++) {
      ...
      // WASM-PORT: the software device's startup runs GFXDDSurface::init
      // (DirectDraw); under the shim that leaves state that hangs the later
      // OGL Surface::create. The software rasterizer can't run on wasm anyway
      // (its asm entry points trap), so never start it in the browser — OpenGL
      // is the only device. enumerateDevices/allocateDevice are guarded too.
...
FreeDeviceManager()
{
   if(!--referenceCount) {
#ifndef __EMSCRIPTEN__
      GFXSoftwareDevice::shutdown();   // WASM-PORT: never started in the browser
#endif
      OGLDevice::shutdown();
   }
}
...
#ifndef __EMSCRIPTEN__
   // WASM-PORT: software + glide devices don't exist in the browser.
...
#endif
   // OpenGL is the only browser device; its enumerateModes is WASM-safe now
   // (no hidden 2nd GL context / EnumDisplaySettings — see gOGLDev.cpp), so it
   // is reported here and GWDeviceManager can discover "OpenGL" by name.
