...
#include <windows.h>
#include <gl/glu.h>
#include <stdio.h>   // NATIVE-PORT: temp OGL trace logging
...
// WASM-PORT: the engine assumes every poly has <= GFX_MAX_POLY_VERTEX (100) verts and
// AssertFatal-guards it (incCurrVertexIndex), but that assert is a no-op under NDEBUG.
// ncity.dis has interior surfaces whose winding exceeds 100 verts, so the per-poly GL
// emit arrays (vertex/color/texcoord/haze, all sized sm_maxNumVertices) overran at render
// time -> heap-buffer-overflow (ASAN: READ of size 4 one past the 100*16 vertex array, in
// the EmitPoly/EmitPoly2Pass draw loops). Enlarge the buffer so real interior polys fit; a
// hard saturating clamp in incCurrVertexIndex() guards anything still larger (renders that
// one poly degenerate rather than corrupting the heap).
int Surface::sm_maxNumVertices = 1024;
#else
...
// (its found==false path would also wrongly fail). SDL owns the canvas size.
// NATIVE-PORT: also skip it natively. The client runs WINDOWED (the user's config), so an exclusive-
// fullscreen mode switch is wrong; worse, the found==false path rejects any non-display-mode size
// (e.g. the windowed 904x718) -> Surface::create returned NULL -> native OpenGL never worked. The GL
// context is created directly on the existing window below. (Re-introduce a real windowed/fullscreen
// flag here if true exclusive-fullscreen OpenGL is ever wanted.)
#if 0
   BOOL   test    = TRUE;
   bool   found   = false;
   UInt32 modeNum = 0;
...
Surface::flip()
{
   if (m_flareBlend != 0.0f) {
      ...
      // WASM-PORT: GL_POLYGON aborts in emscripten's glemu (it supports modes
      // only up to GL_TRIANGLE_FAN) — a convex quad fan is identical here. And
      // the constant glColor must precede glBegin: inside begin/end glemu
      // interleaves one color slot instead of latching a current color, so a
      // single color + 4 verts desyncs the buffer (same fix as DrawRect_f).
   ...
   if (m_alphaBlend != 0.0f) {
      ...
      // WASM-PORT: GL_POLYGON -> GL_TRIANGLE_FAN and color before glBegin
      // (see the flare overlay above and DrawRect_f for the glemu rationale).
                     ...
                     Bool        /*in_rsvPal*/)
{
   ...
   // WASM-PORT: NULL-safe. OGLDevice::enable() runs setResolution -> setPalette
   // with a NULL palette at canvas-create time (before the world palette is
   // assigned), so the hazeColor deref below would fault. The trailing
   // `return (lpPalette != NULL)` shows NULL was always meant to be tolerated.
   if (lpPalette == NULL)
      return false;
