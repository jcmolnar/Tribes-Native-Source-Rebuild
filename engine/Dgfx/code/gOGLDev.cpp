...
OGLDevice::enumerateModes(GFXDeviceDescriptor* out_pDevDesc)
{
   ...
   // WASM-PORT: initRendererName() creates a 2nd hidden Win32 GL window+context
   // and refreshResList() enumerates display modes via EnumDisplaySettings —
   // neither maps onto the single SDL canvas. Report a fixed renderer name and
   // a canned resolution list so the device is discoverable by GWDeviceManager.
   if (g_oglDisabled == true)
      return false;
   if (g_pRendererName == NULL)
      g_pRendererName = "WebGL (OpenGL ES)";
   if (sm_resVector.size() == 0) {
      static const int rs[][2] = { {640,480}, {800,600}, {1024,768}, {1280,1024} };
      for (int k = 0; k < 4; k++) {
         ResSpecifier r; r.res.x = rs[k][0]; r.res.y = rs[k][1]; r.isVirtual = false;
         sm_resVector.push_back(r);
      }
   }
   out_pDevDesc->flags           = GFXDeviceDescriptor::runsWindowed |
                                   GFXDeviceDescriptor::runsFullscreen;
