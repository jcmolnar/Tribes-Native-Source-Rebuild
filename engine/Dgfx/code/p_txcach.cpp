...
// WASM-PORT: interior lightmap surfaces can be up to 255x255 (ITRLighting::Surface::mapSize is a
// Point of UInt8 x,y) = 65025 pixels, but the largest bucket here was only 22000. Under -DNDEBUG the
// AssertFatal(numPixels <= maxMaxSize) in allocate() compiles to nothing, so a lightmap bigger than
// 22000 px walked `count` off the end of arrays[] and returned a WILD pointer; buildLightMap()'s
// memcpy/decompress then wrote up to ~130KB to that wild address, corrupting the heap. The corruption
// surfaced much later as a "memory access out of bounds" trap in UNRELATED code (SDL's event queue,
// SDL_PeepEventsInternal) the moment an interior like ncity.dis loaded. Add a bucket that covers the
// true maximum so allocate() always returns a buffer big enough for the copy.
static int sizes[] = { 32, 80, 200, 400, 1100, 5300, 22000, 65536, 0 };
...
GFXLightMap* GFXLightMapCache::allocate(int numPixels)
{
   ...
   //  Find smallest lightmap buffer. WASM-PORT: bound `count` to the last real bucket so an
   //  oversized request can never walk off the end of arrays[] and return a wild pointer (the
   //  AssertFatal above is a no-op under -DNDEBUG). With the 65536 bucket this guard never trips for
   //  interior lightmaps (max 255x255), but it keeps any future oversized caller contained.
   while (count < numArrays - 1 && arrays[count].maxSize < numPixels)
      count++;
