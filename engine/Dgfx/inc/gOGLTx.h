...
/* WASM-PORT: forward-declaring a nested struct is illegal in ISO C++;
   include the real definition instead */
#include "g_bitmap.h"
struct GFXLightMap;
class  GFXPalette;
...
class Surface::TextureCache
{
  ...
  public:
   ...
   // WASM-PORT: WebGL exposes no SGIS_multitexture extension entry points, so the
   // m_multiTexExtSGI.glMTexCoord2fv pointer is never a valid callable here (it read back as
   // non-null garbage under wasm, so the SGI path ran and did a bad indirect call -> "null
   // function" trap while rendering interior lightmap surfaces). Force false so the renderer
   // uses its core-GL 2-pass fallback (EmitPoly2Pass else-branch) instead of the extension.
   bool supportsSGIMultiTexture() const {
#ifdef __EMSCRIPTEN__
      return false;
#else
      return m_multiTexExtSGI.glMTexCoord2fv != NULL;
#endif
   }
...
#ifdef __EMSCRIPTEN__
   // WASM-PORT: do NOT skip on the per-instance m_transpEnabled cache. Multiple TextureCache
   // instances share ONE GL context but each tracks m_transpEnabled independently, so the cache
   // desyncs from the global GL_ALPHA_TEST state: instance A enables alpha-test (its cache=Enabled),
   // then instance B (cache inits Disabled) calls setTransparent(false) -> "already Disabled" ->
   // the glDisable is SKIPPED -> A's alpha test leaks onto B's draws -> GL_ALPHA_TEST (GREATER 0.65)
   // discards low-alpha texels = the green screen-door DITHER on interiors/objects. Always emit the
   // GL call so the real GL state matches the request, regardless of this instance's stale cache.
   if (in_transp) { glEnable(GL_ALPHA_TEST); glAlphaFunc(GL_GREATER, GLclampf(0.65)); }
