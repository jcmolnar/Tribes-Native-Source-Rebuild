...
class GFXDevice
{
  ...
  public:
   ...
   virtual bool    setResolution(Point2I &res) = 0; /* WASM-PORT: NULL pure-specifier */
   virtual bool    nextRes();
   virtual bool    prevRes();

   virtual bool enable(GFXPalette *palette, DWORD flags) = 0; /* WASM-PORT: NULL pure-specifier */
   virtual void disable() = 0; /* WASM-PORT: NULL pure-specifier */   
   virtual bool modeSwitch(bool fullscreen) = 0; /* WASM-PORT: NULL pure-specifier */

   virtual void flip(const RectI *rect=NULL) = 0; /* WASM-PORT: NULL pure-specifier */
   virtual void lock(const GFXLockMode in_lockMode = GFX_NORMALLOCK) = 0; /* WASM-PORT: NULL pure-specifier */
   virtual void unlock()            = 0; /* WASM-PORT: NULL pure-specifier */
   virtual GFXSurface* getSurface() = 0; /* WASM-PORT: NULL pure-specifier */
   virtual void messageSurface(const int   in_argc,
                                       const char* in_argv[]) = 0; /* WASM-PORT */

   virtual Int32 getDeviceId() = 0; /* WASM-PORT: NULL pure-specifier */ 
};
