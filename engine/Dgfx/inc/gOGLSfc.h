...
Surface::incCurrVertexIndex()
{
   ...
   // WASM-PORT: the AssertFatal below is a no-op under NDEBUG, so a poly with more verts
   // than sm_maxNumVertices would index the emit arrays out of bounds (getCurrentVertex /
   // the EmitPoly draw loops). Saturate at the last slot so the buffer can never overflow
   // (the enlarged sm_maxNumVertices means real interior polys never reach this point).
   if (m_currVertexIndex >= sm_maxNumVertices)
      m_currVertexIndex = sm_maxNumVertices - 1;
#endif
