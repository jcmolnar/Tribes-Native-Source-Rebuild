...
// NATIVE-PORT: scripts use isFunction(name) for feature detection; without it those checks return
// false and gate off features (e.g. the Kronos HUD setup). Return True if name resolves to a
// registered function/command (same lookup the call dispatch uses).
static const char *c_isFunction(CMDConsole *, int, int argc, const char **argv)
{
   if(argc != 2)
      return "False";
   Dictionary::Entry *ent = gEvalState.frames[0]->lookup(stringTable.insert(argv[1]));
   return (ent && ent->func) ? "True" : "False";
};
...
CMDConsole::CMDConsole()
{
   ...
   addCommand(0, "isFunction", c_isFunction);   // NATIVE-PORT: feature-detection used by Kronos HUD scripts
