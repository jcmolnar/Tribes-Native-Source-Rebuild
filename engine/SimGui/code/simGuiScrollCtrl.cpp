...
extern "C" void* sbrk(long);   // WASM-PORT: heap-top probe for the wild-child-pointer guard
static inline bool wasmCtrlSane(const void* p)
{
   char* heapTop = (char*)sbrk(0);
   return ((char*)p >= (char*)4096 && (char*)p < heapTop);
}
#endif
...
bool ScrollCtrl::calcChildExtents(Point2I *pos, Point2I *ext)
{
   ...
   // NATIVE-PORT: contentCtrl can be NULL while the control tree is still being built (onAdd ->
   // setSize -> resize -> computeSizes fires before setContentControl). Guard it (the WASM build
   // hit the partly-built variant of this below via wasmCtrlSane).
   if(!contentCtrl || !contentCtrl->size())
      return false;
   else
   {
      Control *ctrl = (Control *) (contentCtrl->front());
#ifdef __EMSCRIPTEN__
      // WASM-PORT: under the ASAN diagnostic build the redzone allocator shifts the heap so a
      // corrupted/partly-built child pointer in contentCtrl traps here ("memory access out of
      // bounds") during shell boot, BEFORE we can reach the interior render we are trying to ASAN.
      // Skip insane child pointers so boot proceeds; harmless in the normal build (front() is sane).
      if (!wasmCtrlSane(ctrl)) return false;
#endif
      *ext = ctrl->extent;
      *pos = ctrl->position;
...
{
   // NATIVE-PORT: bail if the content control isn't attached yet. onAdd -> setSize -> resize ->
   // childResized -> computeSizes can fire before setContentControl during shell GUI construction,
   // and the sizing helpers (calcChildExtents/calcScrollRects) deref contentCtrl unconditionally.
   if(!contentCtrl)
      return;
...
void ScrollCtrl::inspectRead(Inspect *insp)
{
   ...
   insp->read(IDITG_HANDLE_ARROW_KEYS, willFirstRespond); // WASM-PORT: lvalue casts
   insp->read(IDITG_CONST_THUMB, useConstantHeightThumb);
