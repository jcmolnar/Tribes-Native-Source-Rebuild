                         ...
                         const Box2I& updateRect)
{
   ...
   // WASM-PORT: Control::onRender (Parent) fills the WHOLE control rect with fillColor when
   // mbOpaque is set — for a 3D viewport that opaque fill lands AFTER the world render and
   // OVERPAINTS the just-rasterized scene black (the long-hunted "renders but never visible"
   // bug; geometry emits through EmitPoly2Pass fine, then this clobbers it). A TSControl is a
   // 3D view: it must never opaque-fill over its own scene. Suppress the opaque fill for the
   // Parent call (keep border + child controls), regardless of how the .gui authored mbOpaque.
   {
      BYTE savedOpaque = mbOpaque;
      mbOpaque = FALSE;
      Parent::onRender(sfc, offset, updateRect);
      mbOpaque = savedOpaque;
   }
#else
   Parent::onRender(sfc, offset, updateRect);
#endif
