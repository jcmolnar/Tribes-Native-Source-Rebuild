...
void Plugin::init()
{
   ...
   console->addCommand(GuiGetExtent,"Control::getExtent", this);   // NATIVE-PORT: ScriptGL/vhud HUD queries control size via this
   console->addCommand(GuiGetPosition,"Control::getPosition", this);   // NATIVE-PORT
   console->addCommand(GuiSetPosition,"Control::setPosition", this);   // NATIVE-PORT
   console->addCommand(GuiHudbotAddReplacement,"Hudbot::addReplacement", this);   // NATIVE-PORT: stub
...
const char *Plugin::consoleCallback(CMDConsole *, int id,int argc,const char *argv[])
{
   ...
   // NATIVE-PORT: stub HudBot art replacement (cosmetic; the HUD still renders with default art).
   if(id == GuiHudbotAddReplacement) return "True";
      ...
      case TextListClear:
         ...
         else
         {
            ...
            switch(id)
            {
               ...
               case GuiGetExtent: {   // NATIVE-PORT: return the control's extent as "x y" (ScriptGL/vhud HUD sizing)
                  Point2I e = ctrl->getExtent();
                  return avar("%d %d", e.x, e.y);
               }
               case GuiGetPosition: {   // NATIVE-PORT: return the control's position as "x y"
                  return avar("%d %d", ctrl->position.x, ctrl->position.y);
               }
               case TextListClear: {
      ...
      case TextListAdd:
         ...
         else
         {
            ...
            switch(id)
            {
               ...
               case GuiSetPosition: {   // NATIVE-PORT: set the control's position from "x y"
                  int px = atoi(argv[2]);
                  const char* sp = strchr(argv[2], ' ');
                  int py = sp ? atoi(sp+1) : px;
                  ctrl->position = Point2I(px, py);
                  ctrl->setUpdate();
                  break;
               }
               case TextListAdd: {
