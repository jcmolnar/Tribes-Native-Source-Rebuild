...
{
   cursorON = onOff;
   // NATIVE-PORT: drive the cursor GUI (menus) from Win32 absolute mouse messages instead of the
   // DirectInput relative-delta cursor, which drifts/pins to an edge in windowed mode and depends on
   // device acquisition (dies on minimize/alt-tab). Win32 client coords are absolute and always
   // delivered. Gameplay (cursor off) keeps the DirectInput relative mouse for free-look.
   useWindowsMouseEvents = onOff;
   if(!cursorON)
   {
...
void Canvas::processMouseEvent(int x, int y, int type)
{
   ...
   {
      // NATIVE-PORT: map window-client pixels into the GUI/canvas coordinate space. The Win32 absolute
      // mouse path delivers raw client coords; if the window client size differs from the GUI layout
      // size (surface blitted stretched to the window), the cursor and rendered pointer get offset by
      // exactly that ratio. Scale here so the game cursor tracks the OS pointer. No-op when equal.
      {
         RECT _rc; GetClientRect(getHandle(), &_rc);
         int _cw = _rc.right - _rc.left, _ch = _rc.bottom - _rc.top;
         if(topContentControl && _cw > 0 && _ch > 0)
         {
            Point2I _g = topContentControl->getExtent();
            if(_g.x > 0 && _g.y > 0) { x = x * _g.x / _cw; y = y * _g.y / _ch; }
         }
      }
      cursorPt.x = x;
      cursorPt.y = y;
...
bool Canvas::onSimInputEvent(const SimInputEvent *event)
{
   ...
   switch(event->deviceType)
   {
      ...
      case SI_MOUSE:
         if(!cursorON)
            return false;
         // NATIVE-PORT: when the Win32 absolute mouse path is active (cursor GUI), ignore DirectInput
         // relative mouse events so the two paths don't double-drive / drift the cursor.
         if(useWindowsMouseEvents && event->deviceType == SI_MOUSE)
            return false;
...
void Canvas::render()
{
   ...
   // NATIVE-PORT: match the window client to the render surface once at startup. The surface/GUI can
   // be a larger resolution than the initial 640x480 window (e.g. a Kronos windowed res), which
   // otherwise shows only the top-left of the GUI and offsets the mouse by the size ratio. Resize the
   // window once to fit the surface (ignoreSizeMessages so the WM_SIZE doesn't re-resize the surface).
   {
      static bool _syncedClient = false;
      RECT _rc; GetClientRect(getHandle(), &_rc);
      int _cw = _rc.right - _rc.left, _ch = _rc.bottom - _rc.top;
      if(!_syncedClient && _cw > 0 && _ch > 0 && (_cw != srf->getWidth() || _ch != srf->getHeight()))
      {
         _syncedClient = true;
         ignoreSizeMessages = true;
         setClientSize(Point2I(srf->getWidth(), srf->getHeight()));
...
Canvas::onActivateApp(BOOL f, DWORD t)
{
   if (f == FALSE) {
      ...
      // NATIVE-PORT: the cursor GUI's mouse is driven by Win32 messages (see setCursorON), which
      // resume automatically on restore, so no DirectInput mouse re-acquire is needed here.
   }
