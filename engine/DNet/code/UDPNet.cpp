...
#include "wasm_net.h"   // WASM-PORT: datagrams ride the WebSocket relay, not UDP
#endif
...
UDPTransport::~UDPTransport()
{
   ...
   WSACleanup();
#endif
   // WASM-PORT: transportSocket is a sentinel (not an fd); the relay WebSocket
   // is owned by the JS layer and outlives the transport, so nothing to close.
}

bool UDPTransport::activate(int port)
{
#ifdef __EMSCRIPTEN__
   // WASM-PORT: no OS UDP socket — datagrams ride the relay WebSocket, opened
   // once by the app (wasmNetOpen). "Activation" just marks the transport live;
   // transportSocket is a non-zero sentinel (NOT a file descriptor), so the
   // engine's `transportSocket != 0` liveness checks still work.
   if(port == curPort || (port == 0 && curPort != -1))
      return transportSocket != 0;
   transportSocket = 1;
   curPort = port;
   log_file.log("   activation successful (wasm relay)");
   return true;
#else
...
void UDPTransport::STgetStringFromAddress(const LPVOID addr, char *nameBuf)
{
   ...
   {
      /* WASM-PORT: POSIX in_addr has no s_net/s_host/s_lh/s_impno byte
         fields; derive the dotted quad from s_addr (network order) */
      const unsigned char* b = (const unsigned char*)&addr_in->sin_addr.s_addr;
      sprintf(nameBuf, "IP:%d.%d.%d.%d:%d", b[0], b[1], b[2], b[3],
...
void UDPTransport::process()
{
...
#ifdef __EMSCRIPTEN__
   // WASM-PORT: drain datagrams from the relay WebSocket instead of recvfrom.
   // The relay hands us the source as "a.b.c.d:port"; the rest of the dispatch
   // (game-info vs VC, getVCfromAddress, connect) is identical to the UDP path,
   // so we just synthesize the engine's "IP:..." address form and reuse it.
   for(;;)
   {
      char rawAddr[64];
      int len = wasmNetRecvFrom(rawAddr, sizeof(rawAddr), recvBuf,
                                MaxHeaderSize + MaxPacketSize);
      if(len <= 0)
         break;
      if(gPacketLoss && plRandom.getInt(0, 100) < gPacketLoss)
         continue;

      sprintf(addrBuf, "IP:%s", rawAddr);
   ...
   for(;;)
   {
      ...
      /* WASM-PORT: byte access via s_addr (see STgetStringFromAddress) */
      if(((const unsigned char*)&addr_in.sin_addr.s_addr)[0] == 127 &&
            ((const unsigned char*)&addr_in.sin_addr.s_addr)[1] == 0 &&
            ((const unsigned char*)&addr_in.sin_addr.s_addr)[2] == 0 &&
            ((const unsigned char*)&addr_in.sin_addr.s_addr)[3] == 1 &&
...
#ifdef __EMSCRIPTEN__
   // WASM-PORT: frame the datagram to the relay (dest = addrString minus "IP:").
   {
      const char *ap = addrString;
      if(!strnicmp(ap, "IP:", 3)) ap += 3;
      wasmNetSendTo(ap, data, datalen);
   }
#else
...
#ifdef __EMSCRIPTEN__
      // WASM-PORT: frame the datagram to the relay. addressString is the VC's
      // engine address ("IP:a.b.c.d:port"); the relay wants the bare host:port.
      {
         const char *ap = addressString;
         if(!strnicmp(ap, "IP:", 3)) ap += 3;
         if(wasmNetSendTo(ap, send_data, send_len) < 0)
            return SendError;
         return NoError;
      }
#else
