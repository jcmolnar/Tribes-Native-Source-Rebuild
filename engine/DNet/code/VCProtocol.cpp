...
// DNETDIAG (pt.36, strip before commit): settle whether objective events are LOST at the DNet layer
// (received but rejected/gapped before PacketStream -> EventManager ever sees them) vs never sent. Counts
// total receives, DNet-level drops (bad ordering / bad seqNum), the per-gap "missed N packets" (seqNum
// jumped past lastSeqRecvd+1 = packets the network/relay never delivered), and DataPacket deliveries to the
// session. If the gap counters jump the instant we press O, the objective burst is being lost in transit
// (relay/throttle) before EventManager -> not a parse bug. Read via wasmGetDnetDiag().
static int g_dnetRecv = 0, g_dnetBadOrder = 0, g_dnetBadSeq = 0;
static int g_dnetGapEvents = 0, g_dnetGapPackets = 0, g_dnetDataDeliv = 0, g_dnetLastGap = 0;
static char g_dnetDiagBuf[224];
extern "C" EMSCRIPTEN_KEEPALIVE const char* wasmGetDnetDiag() {
   snprintf(g_dnetDiagBuf, sizeof(g_dnetDiagBuf),
      "recv=%d delivered=%d | DROPS: badOrder=%d badSeq=%d | GAPS: events=%d totalPktsMissed=%d lastGap=%d",
      g_dnetRecv, g_dnetDataDeliv, g_dnetBadOrder, g_dnetBadSeq, g_dnetGapEvents, g_dnetGapPackets, g_dnetLastGap);
   return g_dnetDiagBuf;
}
extern "C" EMSCRIPTEN_KEEPALIVE void wasmClearDnetDiag() {
   g_dnetRecv = g_dnetBadOrder = g_dnetBadSeq = g_dnetGapEvents = g_dnetGapPackets = g_dnetDataDeliv = g_dnetLastGap = 0;
}
#endif
...
void VCProtocol::receive(BYTE *data, int datalen, DWORD recvTime)
{
...
#ifdef __EMSCRIPTEN__
   // DNETDIAG gap: seqNum jumped past lastSeqRecvd+1 => the network/relay never delivered those packets to
   // us (they're not parse-dropped; they never arrived). If this jumps on O, the objective burst was lost
   // in transit before EventManager.
   if((int)seqNum > (int)lastSeqRecvd + 1) {
      g_dnetLastGap = (int)seqNum - (int)lastSeqRecvd - 1;
      g_dnetGapEvents++;
      g_dnetGapPackets += g_dnetLastGap;
   }
#endif
   ...
   else if(pType == DataPacket && myVC->getState() == VC::Connected)
   {
      ...
      recvDataLen = datalen - (recvData - data);
#ifdef __EMSCRIPTEN__
      g_dnetDataDeliv++;   // DNETDIAG: a DataPacket actually reaches the session -> PacketStream -> EventManager
#endif
