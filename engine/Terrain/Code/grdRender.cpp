...
// WASM-PORT diag [TERRSKIP]: Module._wasmSetTerrain(0) skips all terrain rendering. Confirms whether
// the banker-room green spike-fan is terrain overdraw (drawn while the camera is inside the interior).
// Global scope (the C export in test/programshell.cpp does `extern int g_wasmSkipTerrain;`).
int g_wasmSkipTerrain = 0;
#endif
...
{
#ifdef __EMSCRIPTEN__
	if( ::g_wasmSkipTerrain ) return;   // WASM-PORT diag [TERRSKIP]: kill terrain to test green-fan source
#endif
	file = aFile;
