      ...
      case MessageCanvasDevice:
 	   ...
 	   case FlushTextureCache:
         if(argc > 1)
         {
            ...
            {
               SimCanvas *pc = dynamic_cast<SimCanvas*>(obj);
               if(pc) if(GFXSurface* s = pc->getSurface()) s->flushTextureCache();   // NATIVE-PORT: guard NULL canvas/surface
               return 0;
            }
         }
				...
				if ( SimSet *set = dynamic_cast<SimSet *>(g) )
				{
					...
					for ( iter = set->begin(); iter != set->end(); iter++ )
		               ...
		               if(GFXSurface* s = pc->getSurface()) s->flushTextureCache();   // NATIVE-PORT: guard NULL surface
