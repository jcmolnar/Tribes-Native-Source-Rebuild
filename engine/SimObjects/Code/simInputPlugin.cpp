...
void SimInputPlugin::init()
{
   ...
   console->addCommand(PushActionMap,          "pushActionMap", this );   // NATIVE-PORT: scripts push/pop input contexts (in-game play/menu maps)
   console->addCommand(PopActionMap,           "popActionMap", this );
}
...
const char *SimInputPlugin::consoleCallback(CMDConsole *, int id,int argc,const char *argv[])
{
   ...
   {
      //--------------------------------------
      // NATIVE-PORT: the missing pushActionMap/popActionMap console commands. Scripts push a named
      // action map onto the input stack when a GUI/play context opens and pop it when it closes;
      // without these the in-game input context never switches (cursor stays up, no mouse-look).
      case PushActionMap:
      case PopActionMap:
         if(argc == 2)
         {
            Resource<SimActionMap> hMap = SimResource::get(manager)->load(argv[1]);
            SimActionHandler *h = SimActionHandler::find(manager);
            if((bool)hMap && h)
            {
               if(id == PushActionMap) h->push(h, hMap);
               else                    h->pop(h, hMap);
            }
         }
         return "";
