...
#include <stdio.h>   // WASM-PORT diag: [LMPROBE] printf
#endif
#include <console.h>
#include <windows.h>
#include "lock.h"
...
#ifdef __EMSCRIPTEN__
// WASM-PORT perf: freeze interior lightmaps. Animated interior lights (flickering torches/fires)
// invalidate their affected surfaces' lightmaps EVERY frame (installLightState / registerDynamicLight
// -> setLightMapNotValid), forcing a rebuild + full-RGBA glTexImage2D re-upload of ~170 surfaces/frame
// (~0.8ms each => the ~140ms city-view cost). With this set, the invalidations are skipped: each
// surface's lightmap builds once and stays valid, so the flicker animation freezes but the scene stays
// fully lit and renders cheaply. Default ON for a playable port; Module._wasmSetFreezeLightmaps(0)
// restores stock per-frame animated lighting. (The light state/color bookkeeping still runs; only the
// surface-cache invalidation is gated.)
int g_wasmFreezeLightmaps = 1;
#endif
...
#ifdef __EMSCRIPTEN__
      // WASM-PORT perf freeze: validate after one build so the dynamic-lit surface is NOT rebuilt
      // every frame (the full-RGBA re-upload is the city-view bottleneck). Flicker freezes; stays lit.
      if (::g_wasmFreezeLightmaps)
...
#ifdef __EMSCRIPTEN__
   // WASM-PORT diag: why do interior lightmaps multiply to black? Dump mapIndex (-1 => constant
   // mapColor fill), isMissionLit (MissionLighting stub => false => falls back to static maps),
   // and a few decoded UInt16 lightmap texels. If texels are ~0, the static maps are empty
   // (lighting lived in the absent MissionLighting); if non-zero, the upload/convert is the bug.
   // [LMPROBE] silenced: the interior lightmap bug is fixed (static maps decode bright; the GL
   // upload was the bug, now using glTexImage2D unsized-RGBA). Re-enable only if lightmaps regress.
#endif
...
#ifdef __EMSCRIPTEN__
         // WASM-PORT perf freeze: don't re-invalidate every frame for dynamic point lights — the
         // surface keeps its once-built lightmap (frozen flicker) instead of a full re-upload/frame.
         if (!::g_wasmFreezeLightmaps)
#endif
