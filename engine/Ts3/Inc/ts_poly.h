...
TPoly<V>::Plane::Side TPoly<V>::whichSide(Plane& plane)
{
	...
	typename VertexList::iterator end = vertexList.end();   /* WASM-PORT: typename */
	typename VertexList::iterator itr = vertexList.begin(); /* WASM-PORT: typename */
...
{
	for (typename VertexList::iterator itr = vertexList.begin(); /* WASM-PORT: typename */
