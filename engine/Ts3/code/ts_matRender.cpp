...
// WASM-PORT: this observe/decode client has NO runtime lighting (MissionLighting is a stub, the
// SimLightSet is empty), so per-vertex (SmoothShade -> lightPoint) and flat (FlatShade) shading
// evaluate to ~black and MODULATE shape textures/colors to near-black -- player models/items render
// dark while the baked-lightmap interiors are bright. Force CONSTANT full shade for ALL shape faces
// so they draw fullbright (same rationale as the EmitPoly GFX_FILL_TEXTURE white-force on the inline
// path). Shapes have no lightmaps to fall back on, so this is unconditional under wasm.
#undef  SmoothShade
#undef  FlatShade
#define SmoothShade() NoShade()
#define FlatShade()   NoShade()
#endif
