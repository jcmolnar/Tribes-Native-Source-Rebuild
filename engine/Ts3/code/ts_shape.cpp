...
#include <stdio.h>   // WASM-PORT diag: [XFORMDUMP] printf
#endif
...
#if 0 // WASM-PORT diag [XFORMDUMP] DISABLED (transforms proven clean; was flooding the console)
      // WASM-PORT diag [XFORMDUMP]: confirm the per-node DEFAULT transforms decode correctly.
      // The [NODEXFORM] probe showed the default transform quat reading as (0,0,0,0) -- a
      // DEGENERATE rotation (valid identity is (0,0,0,1)). If that is universal here, the
      // version-8 Transform read (Quat16 fRotate: 4xInt16, then Point3F fTranslate) has a
      // packing/stride mismatch on wasm. Dump the first few right after load.
      {
         int nd = fTransforms.size();
         int show = nd < 6 ? nd : 6;
         printf("[XFORMDUMP] ver=%d nTransforms=%d nNodes=%d (first %d):\n",
            version, nd, (int)fNodes.size(), show);
         for( int ti = 0; ti < show; ti++ )
         {
            QuatF q; fTransforms[ti].getRotate( &q );
            Point3F t = fTransforms[ti].getTranslate();
            printf("[XFORMDUMP]  [%d] quat=(%.4f %.4f %.4f %.4f) T=(%.3f %.3f %.3f)\n",
               ti, q.x, q.y, q.z, q.w, t.x, t.y, t.z);
         }
      }
#endif
