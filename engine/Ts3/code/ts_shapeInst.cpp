...
#include <stdio.h>   // WASM-PORT diag: [NODEXFORM] printf
#include <math.h>
#endif
...
#ifdef __EMSCRIPTEN__
   // WASM-PORT diag [NODEXFORM]: the "skin-textured shards from the shape origin" bug.
   // After this node's world transform fpTransform is computed, dump:
   //   - node index + default-transform index + shape version
   //   - the LOCAL default transform (translate + quat) straight out of the file
   //   - the WORLD translate fpTransform->p and the 3 rotation-row lengths.
   // A degenerate node shows up as: row lengths far from 1.0 (bad/zero rotation = collapse
   // to origin), a NaN, or a wildly large world translate. A clean hierarchy has row
   // lengths ~1.0 and small finite translates. Throttled hard so it can't flood.
   if( fpTransform )
   {
      float r0 = sqrtf( fpTransform->m[0][0]*fpTransform->m[0][0] + fpTransform->m[0][1]*fpTransform->m[0][1] + fpTransform->m[0][2]*fpTransform->m[0][2] );
      float r1 = sqrtf( fpTransform->m[1][0]*fpTransform->m[1][0] + fpTransform->m[1][1]*fpTransform->m[1][1] + fpTransform->m[1][2]*fpTransform->m[1][2] );
      float r2 = sqrtf( fpTransform->m[2][0]*fpTransform->m[2][0] + fpTransform->m[2][1]*fpTransform->m[2][1] + fpTransform->m[2][2]*fpTransform->m[2][2] );
      bool degen = !(r0>0.5f&&r0<1.5f) || !(r1>0.5f&&r1<1.5f) || !(r2>0.5f&&r2<1.5f)
                || (r0!=r0) || (r1!=r1) || (r2!=r2);
      if( degen )
      {
         static int s_degenCap = 0;
         if( s_degenCap++ < 40 )
         {
            int dtIdx = fNode.fDefaultTransform;
            QuatF q; getShape().getTransform( dtIdx ).getRotate( &q );
            printf("[NODEDEGEN] dt=%d thread=%d defQuat=(%.4f %.4f %.4f %.4f) worldT=(%.3f %.3f %.3f) rowLen=(%.4f %.4f %.4f)\n",
               dtIdx, fpThread ? 1 : 0, q.x, q.y, q.z, q.w,
               fpTransform->p.x, fpTransform->p.y, fpTransform->p.z, r0, r1, r2);
         }
      }
   }
#endif
