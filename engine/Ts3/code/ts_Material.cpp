...
#include <stdio.h>   // WASM-PORT diag [MATTEX]
#endif
...
#ifdef __EMSCRIPTEN__
          // WASM-PORT diag [MATTEX]: interior/shape walls render white because this rm.load returns
          // NULL (texture file not resolvable -> getTextureMap()==NULL -> registerTexture binds no
          // texture -> noTex). Log the exact filenames that fail so we know which assets/VOLs to
          // mount. Print successes too (throttled) to confirm which DO load.
          {
             static int s_mt = 0;
             bool ok = (GFXBitmap*)fTextureMap != NULL;
             if (!ok || s_mt < 8) {
                printf("[MATTEX] %s map='%s' flags=0x%x\n",
                       ok ? "OK " : "NULL", fParams.fMapFile ? fParams.fMapFile : "(null)",
                       (unsigned)fParams.fFlags);
                s_mt++;
             }
          }
#endif
