...
#include <stdio.h>   // WASM-PORT diag: [DTSPROBE] printf
#include <string.h>  // WASM-PORT diag: [HUMANFACE] strstr
#include <g_bitmap.h>  // WASM-PORT diag: [SKINPAL2] bitmap->paletteIndex
extern int g_wasmDiag;   // WASM-PORT: global diag flag (defined in itrrender.cpp); gates [SLIVER]
#endif
...
#ifdef __EMSCRIPTEN__
// WASM-PORT diag [HUMANDUMP]: key-triggered full vert+face dump for "human"-named CelAnimMeshes.
// Set req=1 via Module._wasmDumpHuman() (exported in test/programshell.cpp); CelAnimMesh::render
// appends each human mesh's full geometry into g_humanDump (read via Module._wasmGetHumanDump()/
// _wasmGetHumanDumpLen() out-of-band so the per-frame console flood can't evict it). File-scope
// (NOT in namespace TS) so render() reaches them by outward lookup -- do NOT add a block-scope
// extern inside namespace TS (that would declare TS::g_... and fail to link).
#define HUMANDUMP_CAP 262144
int  g_wasmDumpHumanReq = 0;
int  g_humanDumpMeshes  = 0;
int  g_humanDumpLen     = 0;
char g_humanDump[HUMANDUMP_CAP];
#endif
...
#ifdef __EMSCRIPTEN__
      // WASM-PORT diag (one-shot): the "skinny triangles from the model origin" bug.
      // Dump frame scale/origin, then for the first few faces: the dequantized SOURCE
      // points and the PROJECTED screen points + clip status. This pins which stage is
      // wrong -- bad dequant (garbage source), bad projection (huge/NaN fPoint), or bad
      // face indexing (faces hitting the slot 0/1 bounds verts). Remove once diagnosed.
      // BOUNDS-CONSISTENCY MODE: the green slivers are a scrambled-DECODE mesh (verts finite &
      // on-screen but tangled). Every frame stores its true min/max model-space bounds in verts
      // [FirstVert] and [FirstVert+1]; the geometry verts [2..nVPF-1] must fit inside them. A
      // mis-decoded mesh has geometry that VIOLATES its own stated bounds. Dequant the stated
      // bounds, then scan the geometry verts; if any falls well outside (>10% slack), dump the
      // shape's nVPF/nFaces/frame + stated bounds + the worst offender. That IS the culprit mesh.
      if( vis != ClipNoneVis && fnVertsPerFrame > 2 )
      {
         Point3F bmin, bmax;
         fVerts[FirstVert + 0].getPoint(bmin, *pScale, *pOrigin);
         fVerts[FirstVert + 1].getPoint(bmax, *pScale, *pOrigin);
         // normalize (min/max may be swapped by sign of scale)
         Point3F lo(bmin), hi(bmax);
         if( hi.x < lo.x ){ float t=lo.x; lo.x=hi.x; hi.x=t; }
         if( hi.y < lo.y ){ float t=lo.y; lo.y=hi.y; hi.y=t; }
         if( hi.z < lo.z ){ float t=lo.z; lo.z=hi.z; hi.z=t; }
         float sx = (hi.x-lo.x)*0.1f + 0.01f;
         float sy = (hi.y-lo.y)*0.1f + 0.01f;
         float sz = (hi.z-lo.z)*0.1f + 0.01f;
         int worst = -1; float worstD = 0.0f; Point3F worstP;
         for( int vi = 2; vi < fnVertsPerFrame; vi++ )
         {
            Point3F p; fVerts[FirstVert + vi].getPoint(p, *pScale, *pOrigin);
            float d = 0.0f;
            if( p.x < lo.x-sx ) d += (lo.x-sx)-p.x; else if( p.x > hi.x+sx ) d += p.x-(hi.x+sx);
            if( p.y < lo.y-sy ) d += (lo.y-sy)-p.y; else if( p.y > hi.y+sy ) d += p.y-(hi.y+sy);
            if( p.z < lo.z-sz ) d += (lo.z-sz)-p.z; else if( p.z > hi.z+sz ) d += p.z-(hi.z+sz);
            if( d > worstD ){ worstD = d; worst = vi; worstP = p; }
         }
         if( worst >= 0 )
         {
            static int s_boundsThrottle = 0;
            if( (s_boundsThrottle++ % 120) == 0 )
               printf("[BOUNDVIOL] nVPF=%d nFaces=%d frame=%d FirstVert=%d bounds=[(%.3f %.3f %.3f)..(%.3f %.3f %.3f)] worstVi=%d d=%.3f p=(%.3f %.3f %.3f) scale=(%.5f %.5f %.5f) origin=(%.3f %.3f %.3f)\n",
                  (int)fnVertsPerFrame, (int)fFaces.size(), frame, FirstVert,
                  lo.x, lo.y, lo.z, hi.x, hi.y, hi.z,
                  worst, worstD, worstP.x, worstP.y, worstP.z,
                  pScale->x, pScale->y, pScale->z, pOrigin->x, pOrigin->y, pOrigin->z);
         }
      }
#endif
...
#ifdef __EMSCRIPTEN__
         // WASM-PORT diag [SKINPAL2]: per-shape palette assignment. The held weapon (sword_skin.bmp,
         // an 8-bit index map w/ a grayscale BMP palette) renders GREEN => a WRONG external palette is
         // applied. palMiss=0, so the bitmap's paletteIndex resolves -- but to the wrong palette. Dump,
         // once per shape (cap 60), the shape name + each textured material's map file + paletteIndex +
         // dims, so we can compare the SWORD's paletteIndex against the BODY's (which colorizes fine).
         {
            // WASM-PORT diag [SKINPAL2]: continuous (throttled) dump of the HUMAN player/bot skin
            // material's REAL loaded bitmap -- paletteIndex, dims, detailLevels, ptr, and a sampled
            // texel (pBits[0] + center) -- to see if the custom armor skin (e.g. rpghide.larmor.bmp)
            // loads with the right palette (1136) and non-black pixels, or if the chunked PBMP load is
            // dropping the palette / yielding black. Only fires for *human* shapes so it never floods.
            const char* nm = shapeInst ? shapeInst->getShapeResource().getFileName() : "(null)";
            bool isHuman = nm && (strstr(nm,"human")||strstr(nm,"Human")||strstr(nm,"HUMAN"));
            static int s_palCap = 0;   // capped: skin data already confirmed perfect; stop flooding so
                                       // [HUMANFACE]/[FACEIDX] (one-shot, at load) stay visible.
            if( isHuman && s_palCap < 2 )
            {
               s_palCap++;
               int nm2 = mats.getMaterialsCount();
               for( int mi = 0; mi < nm2 && mi < 4; mi++ )
               {
                  const Material& mm = mats[mi];
                  int fl = mm.fParams.fFlags & Material::MatFlags;
                  long pidx = -1; int w=0,h=0,dl=0; const void* ptr=0; int t0=-1,tc=-1;
                  if( fl == Material::MatTexture ) {
                     const GFXBitmap* bm = mm.getTextureMap();
                     if( bm ) {
                        pidx = (long)bm->paletteIndex; w=bm->getWidth(); h=bm->getHeight();
                        dl=bm->detailLevels; ptr=bm->pBits;
                        if( bm->pBits && w>0 && h>0 ) { t0=(int)bm->pBits[0]; tc=(int)bm->pBits[(h/2)*bm->stride + w/2]; }
                     }
                  }
                  printf("[SKINPAL2] shape=\"%s\" mat[%d] flags=%d map=\"%s\" palIdx=%ld %dx%d dl=%d pBits=%p texel[0]=%d texel[c]=%d\n",
                     nm?nm:"(noname)", mi, fl, mm.fParams.fMapFile, pidx, w, h, dl, ptr, t0, tc);
               }
            }
         }

         // WASM-PORT diag [FACEIDX]: shards are on-screen finite verts in WRONG triangles (connectivity).
         // A face index of 0/1 references the frame's 2 BBOX verts (slot first+0/+1) -> stretches a
         // triangle to a bbox corner = the fan apex. An index >= fnVertsPerFrame reads a STALE point-array
         // slot (only [first, first+nVPF) were written by addPoints). Print ONLY on such an anomaly (plus
         // the first few shapes as a baseline) so the bad shape is the only thing that speaks -- no flood.
         {
            int minVI = 1<<30, maxVI = -(1<<30), nLow = 0, nOOR = 0;
            for( Vector<Face>::const_iterator f = fFaces.begin(); f != fFaces.end(); f++ )
               for( int k = 0; k < 3; k++ ) {
                  int vi = f->fVIP[k].fVertexIndex;
                  if( vi < minVI ) minVI = vi;
                  if( vi > maxVI ) maxVI = vi;
                  if( vi < 2 ) nLow++;
                  if( vi >= (int)fnVertsPerFrame ) nOOR++;
               }
            static int s_faceIdx = 0;
            if( nLow || nOOR || s_faceIdx < 3 ) {
               s_faceIdx++;
               printf("[FACEIDX] nVPF=%d nFaces=%d FirstVert=%d frame=%d first=%d vIdx[min=%d max=%d] nLow(<2)=%d nOOR(>=nVPF)=%d\n",
                  (int)fnVertsPerFrame, (int)fFaces.size(), FirstVert, frame, first, minVI, maxVI, nLow, nOOR);
            }
         }
#endif

#ifdef __EMSCRIPTEN__
         // WASM-PORT diag [HUMANFACE]: threshold-independent connectivity dump for the humanoid body.
         // [SLIVER] only catches LONG (>60px) screen slivers, so a close-up small model can hide a
         // topology scramble. Dump rpgmalehuman ONCE: face count + index range/anomalies + the first
         // 12 face vertex-index triples + model-space positions of faces 0..3 so we can see by hand
         // whether the triangles are sane or mis-grouped.
         {
            const char* hn = shapeInst ? shapeInst->getShapeResource().getFileName() : 0;
            static int s_humanDone = 0;
            if( hn && strstr(hn, "rpgmalehuman") && s_humanDone < 1 && fFaces.size() > 50 ) {
               s_humanDone++;
               int minVI = 1<<30, maxVI = -(1<<30), nLow = 0, nOOR = 0;
               for( Vector<Face>::const_iterator f = fFaces.begin(); f != fFaces.end(); f++ )
                  for( int k = 0; k < 3; k++ ) {
                     ...
                     if( vi < 2 ) nLow++; if( vi >= (int)fnVertsPerFrame ) nOOR++;
                  }
               printf("[HUMANFACE] shape=\"%s\" nVPF=%d nFaces=%d firstVert=%d vIdx[min=%d max=%d] nLow(<2)=%d nOOR(>=nVPF)=%d\n",
                  hn, (int)fnVertsPerFrame, (int)fFaces.size(), FirstVert, minVI, maxVI, nLow, nOOR);
               int fc = 0;
               for( Vector<Face>::const_iterator f = fFaces.begin(); f != fFaces.end() && fc < 12; f++, fc++ ) {
                  int a=f->fVIP[0].fVertexIndex, b=f->fVIP[1].fVertexIndex, c=f->fVIP[2].fVertexIndex;
                  if( fc < 4 ) {
                     Point3F p0,p1,p2;
                     fVerts[FirstVert+a].getPoint(p0,*pScale,*pOrigin);
                     fVerts[FirstVert+b].getPoint(p1,*pScale,*pOrigin);
                     fVerts[FirstVert+c].getPoint(p2,*pScale,*pOrigin);
                     printf("[HUMANFACE]  f%d vIdx=(%d,%d,%d) mat=%d P0=(%.3f,%.3f,%.3f) P1=(%.3f,%.3f,%.3f) P2=(%.3f,%.3f,%.3f)\n",
                        fc, a,b,c, f->fMaterial, p0.x,p0.y,p0.z, p1.x,p1.y,p1.z, p2.x,p2.y,p2.z);
                  } else {
                     printf("[HUMANFACE]  f%d vIdx=(%d,%d,%d) mat=%d\n", fc, a,b,c, f->fMaterial);
                  }
               }
            }
         }
...
#ifdef __EMSCRIPTEN__
         // WASM-PORT diag [HUMANDUMP]: on Module._wasmDumpHuman(), append EVERY "human"-named mesh's
         // full geometry (per-vert MODEL xyz + SCREEN xy + clip status; per-face index triple + mat)
         // into the heap buffer. No face-count gate (player armor is many small sub-meshes). Captures
         // up to 24 meshes then stops. Read out-of-band via Module._wasmGetHumanDump(). Globals are
         // file-scope (above namespace TS) -- referenced directly, no block-scope extern.
         {
            const char* dn = shapeInst ? shapeInst->getShapeResource().getFileName() : 0;
            if( g_wasmDumpHumanReq && g_humanDumpMeshes < 24 && dn &&
                (strstr(dn,"human")||strstr(dn,"Human")||strstr(dn,"HUMAN")||
                 strstr(dn,"dagger")||strstr(dn,"sword")||strstr(dn,"Dagger")||strstr(dn,"Sword")) &&
                g_humanDumpLen < HUMANDUMP_CAP - 4096 )
            {
               g_humanDumpMeshes++;
               int& L = g_humanDumpLen; const int cap = HUMANDUMP_CAP;
               #define HD_APP(...) do{ if(L<cap-1){ int n=snprintf(g_humanDump+L,cap-L,__VA_ARGS__); if(n>0){ L += (n<cap-1-L)?n:(cap-1-L); } } }while(0)
               float bx0=1e30f,by0=1e30f,bx1=-1e30f,by1=-1e30f; int nClip=0;
               for( int vi=0; vi<(int)fnVertsPerFrame; vi++ ){
                  TransformedVertex* v=&pa->getTransformedVertex(first+vi);
                  if(ClipMask & v->fStatus){ nClip++; continue; }
                  if(v->fPoint.x<bx0)bx0=v->fPoint.x; if(v->fPoint.x>bx1)bx1=v->fPoint.x;
                  if(v->fPoint.y<by0)by0=v->fPoint.y; if(v->fPoint.y>by1)by1=v->fPoint.y;
               }
               HD_APP("=== MESH#%d \"%s\" nVPF=%d nFaces=%d first=%d FirstVert=%d frame=%d scale=(%.5f,%.5f,%.5f) origin=(%.3f,%.3f,%.3f) scrBBox=[(%.0f,%.0f)..(%.0f,%.0f)] nClip=%d ===\n",
                  g_humanDumpMeshes, dn, (int)fnVertsPerFrame, (int)fFaces.size(), first, FirstVert, frame,
                  pScale->x,pScale->y,pScale->z, pOrigin->x,pOrigin->y,pOrigin->z, bx0,by0,bx1,by1, nClip);
               for( int vi=0; vi<(int)fnVertsPerFrame; vi++ ){
                  Point3F mp; fVerts[FirstVert+vi].getPoint(mp,*pScale,*pOrigin);
                  TransformedVertex* v=&pa->getTransformedVertex(first+vi);
                  // fPoint.z is invW (1/w) -- a tiny/negative invW = near-plane grazing => stretch.
                  HD_APP("V%d M(%.3f,%.3f,%.3f) S(%.1f,%.1f) invW=%.6g trW=%.4g st%x\n",
                     vi, mp.x,mp.y,mp.z, v->fPoint.x, v->fPoint.y, v->fPoint.z, v->fTransformedPoint.w, (unsigned)v->fStatus);
               }
               int fc=0;
               for( Vector<Face>::const_iterator f=fFaces.begin(); f!=fFaces.end(); f++,fc++ )
                  HD_APP("F%d %d,%d,%d m%d\n", fc, f->fVIP[0].fVertexIndex, f->fVIP[1].fVertexIndex, f->fVIP[2].fVertexIndex, f->fMaterial);
               #undef HD_APP
            }
         }

         // WASM-PORT diag [SLIVER]: find the shape + faces that produce the green skinny-triangle
         // slivers. For each unclipped face compute its screen-space triangle; flag a SLIVER when the
         // triangle's min-height (area/longestEdge) is tiny while its longest edge is large (a thin
         // splinter). Then ask the discriminator: does any sliver vertex sit FAR OUTSIDE the model's
         // own screen bbox (= a FLUNG / wrongly-referenced vertex) or are all 3 inside it but collinear
         // (= wrong CONNECTIVITY among valid verts)? Dump shape name + worst sliver's indices/coords.
         {
            // model screen bbox over all this frame's verts in the point array:
            float bbMinX=1e30f, bbMinY=1e30f, bbMaxX=-1e30f, bbMaxY=-1e30f;
            for( int vi = 0; vi < (int)fnVertsPerFrame; vi++ ) {
               TransformedVertex *v = &pa->getTransformedVertex(first + vi);
               if( ClipMask & v->fStatus ) continue;   // skip clipped (unprojected) verts
               if( v->fPoint.x < bbMinX ) bbMinX = v->fPoint.x;
               if( v->fPoint.x > bbMaxX ) bbMaxX = v->fPoint.x;
               if( v->fPoint.y < bbMinY ) bbMinY = v->fPoint.y;
               if( v->fPoint.y > bbMaxY ) bbMaxY = v->fPoint.y;
            }
            int   sliverCount = 0, worstFace = -1, worstFlungK = -1, worstMat = -1;
            float worstEdge = 0.0f, worstHeight = 0.0f, worstFlungDist = 0.0f;
            int   wIdx[3]; float wX[3], wY[3];
            // near-clip long-sliver tracking (the held-weapon-near-camera case)
            int   nearFaces = 0, nWorstFace = -1; float nWorstSpan = 0.0f;
            int   nIdx[3]; float nX[3], nY[3]; int nNearK = -1;
            int   fi = 0;
            for( Vector<Face>::const_iterator f = fFaces.begin(); f != fFaces.end(); f++, fi++ )
            {
               TransformedVertex *v0 = &pa->getTransformedVertex(first + f->fVIP[0].fVertexIndex);
               TransformedVertex *v1 = &pa->getTransformedVertex(first + f->fVIP[1].fVertexIndex);
               TransformedVertex *v2 = &pa->getTransformedVertex(first + f->fVIP[2].fVertexIndex);
               // ---- near-clip path: 1-2 verts ClipNear, face not fully off-screen ----
               int nearN = ((v0->fStatus&ClipNear)?1:0)+((v1->fStatus&ClipNear)?1:0)+((v2->fStatus&ClipNear)?1:0);
               if( nearN >= 1 && nearN <= 2 )
               {
                  nearFaces++;
                  TransformedVertex* vv[3] = {v0,v1,v2};
                  float fx[3]={v0->fPoint.x,v1->fPoint.x,v2->fPoint.x}, fy[3]={v0->fPoint.y,v1->fPoint.y,v2->fPoint.y};
                  int fidx[3]={f->fVIP[0].fVertexIndex,f->fVIP[1].fVertexIndex,f->fVIP[2].fVertexIndex};
                  float span=0; int nk=-1;
                  for(int a=0;a<3;a++)for(int b=a+1;b<3;b++){ float d=sqrtf((fx[a]-fx[b])*(fx[a]-fx[b])+(fy[a]-fy[b])*(fy[a]-fy[b])); if(d>span)span=d; }
                  for(int k=0;k<3;k++) if(vv[k]->fStatus&ClipNear){ nk=k; break; }
                  if( span > nWorstSpan ){ nWorstSpan=span; nWorstFace=fi; nNearK=nk; for(int k=0;k<3;k++){nIdx[k]=fidx[k];nX[k]=fx[k];nY[k]=fy[k];} }
                  continue;
               }
               if( ClipMask & (v0->fStatus | v1->fStatus | v2->fStatus) ) continue; // only screen-space faces
               float ax=v0->fPoint.x, ay=v0->fPoint.y, bx=v1->fPoint.x, by=v1->fPoint.y, cx=v2->fPoint.x, cy=v2->fPoint.y;
               ...
               float area2 = fabsf((bx-ax)*(cy-ay) - (cx-ax)*(by-ay)); // 2*area
               float height = longest>0.001f ? area2/longest : 0.0f;   // min altitude
               if( longest > 60.0f && height < 1.5f )                   // long + paper-thin = sliver
               {
                  sliverCount++;
                  // how far does the most-outside vertex fall beyond the model bbox?
                  float fx[3]={ax,bx,cx}, fy[3]={ay,by,cy}; int fidx[3]={f->fVIP[0].fVertexIndex,f->fVIP[1].fVertexIndex,f->fVIP[2].fVertexIndex};
                  for( int k=0;k<3;k++ ){
                     float d=0;
                     if( fx[k]<bbMinX ) d+=bbMinX-fx[k]; else if( fx[k]>bbMaxX ) d+=fx[k]-bbMaxX;
                     if( fy[k]<bbMinY ) d+=bbMinY-fy[k]; else if( fy[k]>bbMaxY ) d+=fy[k]-bbMaxY;
                     if( d>worstFlungDist ){ worstFlungDist=d; worstFlungK=k; }
                  }
                  if( longest > worstEdge ){
                     worstEdge=longest; worstHeight=height; worstFace=fi; worstMat=f->fMaterial;
                     for(int k=0;k<3;k++){ wIdx[k]=fidx[k]; wX[k]=fx[k]; wY[k]=fy[k]; }
                  }
               }
            }
            if( sliverCount > 0 || (nWorstSpan > 200.0f) )
            {
               static int s_sliverThrottle = 0;
               if( (s_sliverThrottle++ % 60) == 0 )
               {
                  const char* nm = shapeInst ? shapeInst->getShapeResource().getFileName() : "(null)";
                  // model-space positions of the worst thin face's 3 verts (degenerate decode test):
                  Point3F m0(0,0,0), m1(0,0,0), m2(0,0,0);
                  if( worstFace >= 0 ) {
                     fVerts[FirstVert + wIdx[0]].getPoint(m0, *pScale, *pOrigin);
                     fVerts[FirstVert + wIdx[1]].getPoint(m1, *pScale, *pOrigin);
                     fVerts[FirstVert + wIdx[2]].getPoint(m2, *pScale, *pOrigin);
                  }
                  // material of the worst sliver face (why is it green?):
                  // WASM-PORT diag: print the REAL loaded-bitmap paletteIndex + dims (getTextureMap),
                  // NOT the material fParams.fIndex (which is 0/unused for a textured material and
                  // misled an earlier diagnosis). bmPal=the GFXBitmap::paletteIndex actually resolved.
                  int matFlags=-1, matIndex=-1, mr=-1, mg=-1, mb=-1; const char* mapfile="";
                  long bmPal=-2; int bmW=-1, bmH=-1, bmDL=-1;
                  if( worstMat >= 0 ) {
                     int mid = matRemap ? matRemap[worstMat] : worstMat;
                     if( mid >= 0 && mid < (int)mats.getMaterialsCount() ) {
                        const Material& mm = mats[mid];
                        matFlags = mm.fParams.fFlags & Material::MatFlags;
                        matIndex = mm.fParams.fIndex;
                        mr = mm.fParams.fRGB.fRed; mg = mm.fParams.fRGB.fGreen; mb = mm.fParams.fRGB.fBlue;
                        mapfile = mm.fParams.fMapFile;
                        const GFXBitmap* bm = mm.getTextureMap();
                        if( bm ) { bmPal=(long)bm->paletteIndex; bmW=bm->getWidth(); bmH=bm->getHeight(); bmDL=bm->detailLevels; }
                     }
                  }
                  // DECISIVE: flungDist = how far the worst sliver vert projects OUTSIDE the model's
                  // own screen bbox. Large flungDist => a single vertex is mis-PROJECTED (transform/
                  // projection bug) and many faces fan to it. ~0 => verts in-bbox but collinear
                  // (connectivity). Dump the worst face's 3 MODEL coords + 3 SCREEN coords + which vert
                  // (flungK) is the apex + screen bbox, so we can see exactly which vertex flies off.
                  if(::g_wasmDiag)   // WASM-PORT: gate this per-frame [SLIVER] flood (default off)
                  printf("[SLIVER] shape=\"%s\" nFaces=%d slivers=%d face=%d edge=%.0f h=%.2f flungK=%d flungDist=%.0f"
                         " sbbox=[(%.0f,%.0f)..(%.0f,%.0f)] vIdx=(%d,%d,%d)"
                         " SCR[(%.0f,%.0f)(%.0f,%.0f)(%.0f,%.0f)] MODEL[(%.3f,%.3f,%.3f)(%.3f,%.3f,%.3f)(%.3f,%.3f,%.3f)] map=\"%s\"\n",
                     nm?nm:"(noname)", (int)fFaces.size(), sliverCount, worstFace, worstEdge, worstHeight,
                     worstFlungK, worstFlungDist, bbMinX,bbMinY,bbMaxX,bbMaxY,
                     wIdx[0],wIdx[1],wIdx[2], wX[0],wY[0],wX[1],wY[1],wX[2],wY[2],
                     m0.x,m0.y,m0.z, m1.x,m1.y,m1.z, m2.x,m2.y,m2.z, mapfile);
               }
            }
         }
#endif
