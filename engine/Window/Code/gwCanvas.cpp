...
void GWCanvas::initSurfaces()
{
   ...
   // WASM-PORT: the software rasterizer doesn't exist in the browser — its asm
   // entry points trap and GFXSoftwareDevice::startup's DDraw init hangs (see
   // gdmanag.cpp), so it isn't even enumerated under __EMSCRIPTEN__. OpenGL is
   // the only device. Allocate a single windowed OpenGL device on the one SDL
   // canvas and leave fullscreenDevice NULL — a second device would spin up a
   // second GL context. setFullScreen/surfaceSwap are never driven in-browser.
   windowedDevice = deviceManager.allocateDevice("OpenGL", getAppHandle(), getHandle());
   fullscreenDevice = NULL;
   current = windowedDevice;
#else
   // NATIVE-PORT: boot windowed OpenGL (the KronosHUD/ScriptGL needs it). Debugging the load-in
   // heap-corruption crash this exposes. Don't allocate Software (its DDraw startup hangs OpenGL).
   windowedDevice = deviceManager.allocateDevice("OpenGL", getAppHandle(), getHandle());
   if(!windowedDevice)
...
bool GWCanvas::setFullscreenDevice(const char *name)
{
   ...
   // NATIVE-PORT (crash-harden): allocate the requested device FIRST. The original code disabled +
   // freed the working device before allocateDevice, then NULL-deref'd at current->modeSwitch if the
   // allocation failed (the OpenGL device fails to start natively). Allocate first; if it fails, bail
   // without tearing anything down so the game keeps its current (working) device.
   GFXDevice *newFsDevice = deviceManager.allocateDevice(name, getAppHandle(), getHandle());
   if(!newFsDevice)
   {
      OutputDebugStringA("[NATIVE] setFullscreenDevice: allocateDevice('");
      OutputDebugStringA(name);
      OutputDebugStringA("') failed; keeping current device\n");
      return false;
   }
...
bool GWCanvas::setWindowedDevice(const char *name)
{
   ...
   // NATIVE-PORT (crash-harden): allocate the new device FIRST; bail without teardown if it fails
   // (the OpenGL device fails to start natively / single-device limit), keeping the working device.
   GFXDevice *newWinDevice = deviceManager.allocateDevice(name, getAppHandle(), getHandle());
   if(!newWinDevice)
   {
      OutputDebugStringA("[NATIVE] setWindowedDevice: allocateDevice('");
      OutputDebugStringA(name);
      OutputDebugStringA("') failed; keeping current device\n");
      return false;
   }
...
//------------------------------------------------------------------------------
bool GWCanvas::setResolution(const Point3I &size) // WASM-PORT: const so temps bind
{
   if (!current)
   {
      initSurfaces();
