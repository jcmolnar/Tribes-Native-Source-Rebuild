...
#ifdef __EMSCRIPTEN__
         // WASM-PORT: decode/observe client carries no asset palettes — log which
         // one a real client must mount and keep the ghost so decoding continues.
         if (wasmLogOnce((const char*)fileName)) {
            printf("[WASM-PORT] SimPalette: \"%s\" not mounted (decode continues; a real "
                   "client must preload this palette)\n", (const char*)fileName);
            fflush(stdout);
         }
         return true;
#else
