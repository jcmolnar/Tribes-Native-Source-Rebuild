...
void SimTelnetPlugin::endFrame()
{
   ...
   if(acceptSocket != INVALID_SOCKET)
   {
      ...
      {
   		/* WASM-PORT: POSIX in_addr has no S_un union; use s_addr bytes */
   		console->printf ("Telnet connection from %i.%i.%i.%i",
   				((unsigned char*)&sockAddr.sin_addr.s_addr)[0],	((unsigned char*)&sockAddr.sin_addr.s_addr)[1],
   				((unsigned char*)&sockAddr.sin_addr.s_addr)[2],	((unsigned char*)&sockAddr.sin_addr.s_addr)[3]);
         TelnetClient cl;
         cl.fd = newConnection;
         cl.curPos = 0;
