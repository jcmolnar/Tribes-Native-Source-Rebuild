...
// WASM-PORT diag (KEEP): the last client-side disconnect reason. onConnectionError is the ONLY path the
// client tears its own connection down with a reason (a packet/stream error). If we drop but this stays
// empty, the client did NOT detect an error => the server went silent (timeout/kick) — cross-check with
// wasmGetFrameCount (loop alive?) + the HUD. Read via Module.ccall('wasmGetDropReason','string',[],[]).
static char g_dropReason[256] = {0};
extern "C" EMSCRIPTEN_KEEPALIVE const char* wasmGetDropReason() { return g_dropReason; }
#endif

void CSDelegate::onConnectionError(PacketStream *stream, const char* errorString)
{
#ifdef __EMSCRIPTEN__
   snprintf(g_dropReason, sizeof(g_dropReason), "onConnectionError: \"%s\"", errorString ? errorString : "(null)");
   printf("[DROP] %s\n", g_dropReason), fflush(stdout);
#endif
