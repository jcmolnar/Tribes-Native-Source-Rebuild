...
bool SimInputManager::open()
{
   ...
   // WASM-PORT: no DirectInput in the browser. The manager opens with no
   // DInput devices; input arrives as SimInputEvents injected from the SDL
   // event pump (see the SDL input backend) and flows through onSimInputEvent
   // -> the consumer set / action handler exactly as the DInput path did.
   // Returning true keeps the CLIENT-world path (setWorld) past its
   // AssertFatal("DirectInput init failed").
   return (true);
#else
