...
void RemoteCreateEvent::unpack(SimManager *manager, PacketStream *ps, BitStream *bstream)
{
   ...
   // CRASH-HARDEN: Persistent::create returns NULL for any unregistered ghost tag (tag is a
   // wire-read readInt(10) = 0..1023; game-side classes not compiled into the engine return NULL).
   // Without this check, obj->unpackUpdate() immediately null-derefs and crashes the client.
   if(!obj) { Net::setLastError("Invalid packet."); return; }
   ...
   while(bstream->readFlag())
   {
      ...
      else
      {
         if(!ghostRefs[index].localGhost) // it's a new ghost... cool
         {
            ...
            if(!obj)
            {
               // WASM-PORT: name the unregistered ghost class tag so the browser-port
               // decode harness can mirror it (same diagnostic as netEventManager.cpp).
               // Also dump oNumber + the next bits of the would-be unpackUpdate so the
               // class can be identified by matching its leading field pattern.
               printf("netGhostManager: UNREGISTERED ghost classTag=%d (idSize=%d ghostAlways=%d oNumber=%d)\n",
                      (int)tag, idSize, (int)ghostAlwaysMode, (int)oNumber);
               {
                  int save = bstream->getCurPos();
                  printf("  [ghost-bits] next 128 bits after tag:");
                  for(int bb = 0; bb < 128; bb++) {
                     if((bb & 31) == 0) printf(" ");
                     printf("%d", bstream->readFlag() ? 1 : 0);
                  }
                  printf("\n  [ghost-floats] reinterpret first 32 bits as: ");
                  bstream->setCurPos(save);
                  { float f; bstream->read(&f); printf("float=%g ", f); }
                  bstream->setCurPos(save);
                  { int iv; bstream->read(&iv); printf("int=%d", iv); }
                  printf("\n");
                  bstream->setCurPos(save);
               }
               fflush(stdout);
