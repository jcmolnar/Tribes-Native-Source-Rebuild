...
bool SimTerrain::onAdd()
{
   ...
   {
#ifdef __EMSCRIPTEN__
      // WASM-PORT: load the terrain GEOMETRY but SKIP the CRC verify entirely. The dev .ted
      // (mounted as a vol — its PVOL directory holds the requested rpgbutch5.dtf GFIL GridFile +
      // Lush.dml) will never match the live V0.8.5 server's CRC, and a ghost must render whatever
      // it has and never reject (same render-not-reject rule as the interior load filter). load()
      // resolves the GridFile + materials and builds the ghost terrainFile. If the file isn't
      // mounted load() returns false and we keep the ghost unloaded (RenderImage::render early-outs
      // on a null gridFile), but we STILL fall through to the scene-graph wiring below so
      // SimTerrainId is assigned (Sky::initialize / the sky-band path needs the terrain present).
      bool wasmLoaded = load(tedFile);
      if (wasmLogOnce((const char*)tedFile)) {
         printf("[WASM-PORT] SimTerrain: \"%s\" %s\n", (const char*)tedFile,
                wasmLoaded ? "loaded (rendering)" : "not loadable (ghost kept, won't render)");
         fflush(stdout);
      }
#else
...
void SimTerrain::lightArea( const Box2F & area )
{
   ...
   for( i = 0; i < 3; i++ )
   {
      ...
      /* WASM-PORT: MSVC bound a temporary to a non-const ref here */
      Point2I stripOrigin( iarea.fMin.x, iarea.fMin.y + i - 1 );
      strips[i]->fillStrip( stripOrigin );
   }
   ...
   for( i = 0; i < height; i++ )
   {
      ...
      strips[2] = tmp;
      /* WASM-PORT: MSVC bound a temporary to a non-const ref here */
      Point2I nextOrigin( iarea.fMin.x, iarea.fMin.y + i + 2 );
      tmp->fillStrip( nextOrigin );
   }
