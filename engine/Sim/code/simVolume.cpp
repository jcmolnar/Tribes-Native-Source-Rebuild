...
#ifdef __EMSCRIPTEN__
         // WASM-PORT: the decode/observe client doesn't carry the server's asset
         // vols (they're only needed to RENDER, not to decode the ghost stream).
         // Don't disconnect on a missing vol — log which one a real client would
         // need to mount and keep the ghost registered so decoding continues.
         if (wasmLogOnce((const char*)fileName)) {
            printf("[WASM-PORT] SimVolume: \"%s\" not mounted (decode continues; a real "
                   "client must preload this vol)\n", (const char*)fileName);
            fflush(stdout);
         }
         return true;
#else
