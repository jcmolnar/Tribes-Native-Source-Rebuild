...
bool SimInputDevice::capture()
{
   if (active && !captured && did)      
   {
      ...
      getWindowHandle();
      HRESULT r;
      // NATIVE-PORT: the MOUSE must be NONEXCLUSIVE in a windowed app. DISCL_EXCLUSIVE hides and
      // CONFINES the OS cursor (it freezes in place), which deadlocks the windowed Win32-cursor GUI:
      // the OS cursor can't move, so no WM_MOUSEMOVE fires and the cursor is stuck. Other devices
      // keep the original exclusive-then-nonexclusive behavior.
      DWORD _coopLevel = (deviceType == SI_MOUSE) ? (DISCL_FOREGROUND|DISCL_NONEXCLUSIVE)
                                                  : (DISCL_FOREGROUND|DISCL_EXCLUSIVE);
      r = did->SetCooperativeLevel(hWnd, _coopLevel);
      if (!SUCCEEDED(r))
      {
