...
private:
   ...
   // WASM-PORT: set when a NormalMode (client) stream receives a server packet, cleared on send.
   // Forces a rate-limited (header-only) ack packet to be transmitted even on quiet frames so the
   // server's reliable-event window keeps draining — see checkPacketSend()/handlePacket().
   bool clientAckPending;
#endif
public:
