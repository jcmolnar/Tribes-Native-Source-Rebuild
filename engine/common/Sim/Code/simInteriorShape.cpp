                                    ...
                                    Box3F*        out_pBox)
{
   ...
   // WASM-PORT: guard a non-null instance with NULL current geometry (ghosted interior whose
   // geometry sub-resource didn't load) — getGeometry()->box would read OOB. See the program
   // InteriorShape::buildContainerBox copy for the live trap this fixes.
   if (m_pITRInstance == NULL || m_pITRInstance->getGeometry() == NULL)
      return;
