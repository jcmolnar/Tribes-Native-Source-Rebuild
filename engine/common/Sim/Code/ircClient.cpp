...
void IRCClient::getClientAddress()
{
   ...
   char buf[256];
   // WASM-PORT: musl's in_addr has only s_addr, not the Win16 union fields
   // (s_net/s_host/s_lh/s_impno). Those alias the four address bytes in order,
   // so read them straight out of s_addr (network byte order = octets in order).
   unsigned char *ipb = (unsigned char *) &addr_in.sin_addr.s_addr;
   sprintf(buf, "IP:%d.%d.%d.%d", ipb[0], ipb[1], ipb[2], ipb[3]);
...
void IRCClient::Person::setNick(const char *_nick)
{
   ...
   {
      if (*nick) delete [] nick;
      nick = (_nick && *_nick) ? strnew(_nick) : (char *) "";  // WASM-PORT: literal->char* cast (clang)
...
void IRCClient::Person::setReal(const char *_real)
{
   ...
   {
      if (*real) delete [] real;
      real = (_real && *_real) ? strnew(_real) : (char *) "";  // WASM-PORT: literal->char* cast (clang)
   }
}  
...
void IRCClient::Person::setIdent(const char *_ident)
{
   ...
   {
      if (*identity) delete [] identity;
      identity = (_ident && *_ident) ? strnew(_ident) : (char *) "";  // WASM-PORT: literal->char* cast (clang)
   }
}   
...
{
   if (*topic) delete [] topic;   
   topic = ( t == NULL ) ? (char *) "" : strnew(t);  // WASM-PORT: literal->char* cast (clang)
}   
...
{
   if (*key) delete [] key;   
   key = ( k == NULL ) ? (char *) "" : strnew(k);  // WASM-PORT: literal->char* cast (clang)
}   
