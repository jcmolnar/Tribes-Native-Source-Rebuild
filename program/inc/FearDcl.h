...
// WASM-PORT: the game's global RNG is named `random`, which collides at LINK with libc's
// random() function (data-vs-function symbol type mismatch in wasm-ld). The -U_GNU_SOURCE
// flags hide musl's declaration at compile time, but libc.a(random.o) still gets pulled and
// clashes. An asm label renames just the SYMBOL (call sites keep using `random`), so the
// game global links as `fear_global_random`, distinct from libc random(). Every TU that
// references the global must see this, so a clean program rebuild is required after editing.
#ifdef __EMSCRIPTEN__
extern Random random asm("fear_global_random");
#else
...
// WASM-PORT: Car/Tank/Elevator are stock-Tribes vehicles Kronos never ghosts.
// Placeholder unique tags in the free 548-552 slot so they compile/link; real
// values unknown. If a Kronos ghost ever decodes wrong here, these are suspect.
#define CarPersTag 549
#define TankPersTag 548
#define ElevatorPersTag 550
#define DoorPersTag 552   // WASM-PORT: placeholder; Door is a MoveableBase trigger
...
// WASM-PORT: placeholder tag. Kronos DOES ghost interior shapes (Murch Temple, Control
// Point Thaliel seen in live TargetName decode), so verify this against a real
// FearInteriorShape ghost before trusting unpackUpdate of interiors in the port harness.
#define FearInteriorShapePersTag 523
...
#define HoverPersTag 551   // WASM-PORT: placeholder (Kronos never ghosts Hover); see Car/Tank note above
...
enum EventId
{
	...
	TeamObjectiveEventType = 1113, //  *** DATA-CONFIRMED (full-packet capture re/pktcap_objburst.txt, 2026-06-19):
                               //  the live server sends EVERY objective as tag 1113, framed guaranteed+ordered,
                               //  payload = [objNum:32 LSB][writeString(text)]. Decoded all 26 lines (1,3,4,6..19,
                               //  22..27 incl. the empty lines 19/27 as objNum+empty-string) from one O-burst, each
                               //  with sequential seq. The 2-day "1024 / 2-line desync" was a TAG MISMATCH: our
                               //  reconstructed FearDcl had this at 1024 and MissReset at 1113, so create(1113)
                               //  returned MissReset (0-bit unpack) and DROPPED every objective payload; the dropped
                               //  objNum's low bits (e.g. 7=111) were then misread as a phantom tag-1024 event.
                               //  FIX = tag 1113 here (+ MissReset moved to 1024, not sent by this server) + the
                               //  simple objNum32+readString unpack. See [[objective-wire-format]].
	...
	MissionResetEventType = 1024, //  moved off 1113 (now TeamObjective). Not sent by the live Kronos server; 1024
                              //  is free here (the old "tag 1024" objective events were a desync artifact, not real).
