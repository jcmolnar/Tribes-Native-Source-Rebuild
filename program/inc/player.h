...
public: //private:
	...
	// WASM-PORT: the observe/decode client's hand-written main loop (programshell onIdle) does not run
	// FearGame::clientProcess's per-object loop (main.cpp doesn't even compile), so ghosted players'
	// clientProcess never ticks -> no position interpolation (stepDelta) between packets AND frozen
	// body/arm-fire/weapon animations. This PUBLIC wrapper lets onIdle drive each player's protected
	// clientProcess (interpolation + animation); the same-frame guard mirrors main.cpp:384. clientProcess's
	// decal/footstep block is dmgr-null-guarded so it's safe in the observe client.
	void wasmTickClient(DWORD curTime);
#endif
