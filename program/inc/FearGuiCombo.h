...
namespace FearGui
{
	...
	public:
		...
		virtual void drawText(GFXSurface* sfc, GFXFont* font, const char* text, const Point2I& offset, const Point2I& dest);
		virtual void setSelected(Point2I& cell);
		virtual void addEntry(const char* buf, int id);
		// WASM-PORT: non-virtual so FGGenericComboBox can declare a same-name findEntry that
		// returns Point2I (cell coords) — a hide, not an override-with-different-return (error).
		// Nothing dispatches findEntry polymorphically through an FGComboBox*.
	...
	protected:
		virtual void setBoarder();   // WASM-PORT: was private; derived popups' onAdd call it
		bool onAdd() override;
