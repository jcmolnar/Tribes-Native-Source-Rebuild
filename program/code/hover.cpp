...
//	else
		{
			setRot (rot);
			dynamics (adjust);
			updateMovement (interval);  // WASM-PORT: ShapeBase::updateMovement(slice); Flier/Tank pass interval too
		}
...
// WASM-PORT: minimal pilot-eject. Stock Vehicle::dismount handled the full unmount
// handshake; Hover is the only caller and Kronos never instantiates it, so this just
// releases the pilot and records lastPilot (processCollision ignores collisions with it).
void Hover::dismount()
{
	lastPilot = pilot;
	pilot = NULL;
