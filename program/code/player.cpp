...
bool Player::initResources(GameBase::GameBaseData *in_data)
{
   ...
   // WASM-PORT: the "looks" blend runs 0=look-up .. 0.5=level .. 1=look-down (see updateAnimation:
   // pos = 0.5 + (-viewPitch/MaxPitch)*0.5). This observe/decode client never ticks ghosted players'
   // clientProcess, so updateAnimation never runs and the looks thread stays frozen at its seed. Seed
   // it at LEVEL (0.5) so standing NPCs face forward instead of aiming straight up. (Proven: the
   // [LOOKPITCH] g_wasmLookPitchDbg.updates counter stayed 0 for observed NPCs -> updateAnimation
   // unrun.) For a properly-simulated player updateAnimation overrides this on frame 1, so harmless.
   viewThread->SetSequence( looksSequence, 0.5 );
#else
