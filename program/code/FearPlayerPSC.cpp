...
#else
static int g_netDiag = 0;   // native: NETDIAG trace counter (armed/decremented at runtime; printf goes nowhere in a windowed build)
#endif
...
#ifdef __EMSCRIPTEN__
// WASM-PORT: browser-driven movement. A real client builds curMove's action axes from the
// keyboard command map via processAction(); the harness has no command map wired to the move
// pump, so these globals stand in for it. Set via Module._wasmSetMove(fwd, side, jump[, jet])
// (programshell.cpp). fwd: +forward / -back, side: +right / -left (each -1..1). Consumed in
// clientCollectInput exactly where processAction would set curMove, so the move flows through
// the normal write()/read()/getClientMove() path to the server.
float g_wasmMoveFwd  = 0.0f;
float g_wasmMoveSide = 0.0f;
bool  g_wasmMoveJump = false;
bool  g_wasmMoveJet  = false;
#endif
...
void PlayerPSC::clientCollectInput(DWORD startTime, DWORD endTime)
{
   ...
   if(owner->getStreamMode() != Net::PacketStream::PlaybackMode)
   {
      ...
      {
#ifdef __EMSCRIPTEN__
         // WASM-PORT: stand in for the command map — set the move axes the same way
         // processAction would, so the browser's WASD/jump feed the move pump. Sticky
         // across ticks (like the real action state) so held keys keep moving.
         curMove.forwardAction  = g_wasmMoveFwd  > 0.0f ?  g_wasmMoveFwd  : 0.0f;
         curMove.backwardAction = g_wasmMoveFwd  < 0.0f ? -g_wasmMoveFwd  : 0.0f;
         curMove.rightAction    = g_wasmMoveSide > 0.0f ?  g_wasmMoveSide : 0.0f;
         curMove.leftAction     = g_wasmMoveSide < 0.0f ? -g_wasmMoveSide : 0.0f;
         curMove.jetting        = g_wasmMoveJet;
         curMove.jumpAction     = g_wasmMoveJump;
#endif
         ...
         curMove.turnRot = yawAdd;
#ifdef __EMSCRIPTEN__
         curMove.jumpAction = g_wasmMoveJump;   // WASM-PORT: keep browser jump for trailing ticks
#else
...
// WASM-PORT O-DETACH diag (strip before commit): rings of the last client<->server PlayerPSC state, to
// see how firstMoveSeq / moves / controlObject evolve across the O avatar-detach (remoteSCOM -1) that the
// server rejects with a DNet Disconnect. OUT = our send (writePacket else branch); IN = each PSC we read
// (readPacket client branch: newMode + lastMove + controlObject). Read via Module.ccall(..,'string',..).
static char g_pscDiagBuf[1024] = {0};
struct PscOutRec { int seq; int mv; int ctrl; };
struct PscInRec  { int mode; int last; int ctrl; };
static PscOutRec g_pscOut[24]; static int g_pscOutN = 0;
static PscInRec  g_pscIn[24];  static int g_pscInN = 0;
static void wasmPscOutPush(int seq, int mv, int ctrl){
   int i = g_pscOutN % 24; g_pscOut[i].seq = seq; g_pscOut[i].mv = mv; g_pscOut[i].ctrl = ctrl; g_pscOutN++; }
static void wasmPscInPush(int mode, int last, int ctrl){
   int i = g_pscInN % 24; g_pscIn[i].mode = mode; g_pscIn[i].last = last; g_pscIn[i].ctrl = ctrl; g_pscInN++; }
extern "C" EMSCRIPTEN_KEEPALIVE const char* wasmGetPscDiag(){
   char* p = g_pscDiagBuf; int cap = sizeof(g_pscDiagBuf); int w = 0;
   w += snprintf(p+w, cap-w, "OUT(n=%d):", g_pscOutN);
   int on = g_pscOutN<24?g_pscOutN:24, os = g_pscOutN<24?0:(g_pscOutN%24);
   for(int k=0;k<on && w<cap-1;k++){ PscOutRec&r=g_pscOut[(os+k)%24];
      w += snprintf(p+w,cap-w," %d/m%d/%c", r.seq, r.mv, r.ctrl?'C':'-'); }
   w += snprintf(p+w, cap-w, " || IN(n=%d):", g_pscInN);
   int in = g_pscInN<24?g_pscInN:24, is = g_pscInN<24?0:(g_pscInN%24);
   for(int k=0;k<in && w<cap-1;k++){ PscInRec&r=g_pscIn[(is+k)%24];
      w += snprintf(p+w,cap-w," m%d/L%d/%c", r.mode, r.last, r.ctrl?'C':'-'); }
   return g_pscDiagBuf;
}
#endif
...
bool PlayerPSC::writePacket(BitStream *bstream, DWORD &key)
{
   ...
   else
   {
      ...
      bstream->writeFlag(false);
#ifdef __EMSCRIPTEN__
      wasmPscOutPush((int)firstMoveSeq, moves.size(), controlObject != 0);
#endif
      return true;
   }
}
...
#ifdef __EMSCRIPTEN__
#include <emscripten.h>
// CMDBITS capture (strip before commit): the command-mode PSC packet's raw bits, stored in a GLOBAL so
// it survives console flooding (the GUI switch floods the log with SimVolume/PROF). Read from JS via
// Module.ccall('wasmGetCmdBits','string',[],[]) AFTER triggering a command-mode toggle.
static char g_cmdBitsBuf[320] = {0};
extern "C" EMSCRIPTEN_KEEPALIVE const char* wasmGetCmdBits() { return g_cmdBitsBuf; }
#endif
...
void PlayerPSC::readPacket(BitStream *bstream, DWORD currentTime)
{
   ...
   {
      // RPKT DIAG (strip before commit). ENTER pos/valid tells us if the stream is ALREADY desynced
      // before PSC reads anything (=> the desync is upstream, in a ghost/event demarshaled earlier in
      // this packet) or goes bad inside readPacket. Gated on g_netDiag (armed below on the first non-
      // PlayGui packet) so normal play is silent/fast and the objectives transition bursts full detail.
      bool dbg = (g_netDiag > 0);
      int entryPos = bstream->getCurPos();
      if(dbg) printf("[RPKT] ENTER pos=%d valid=%d\n", entryPos, (int)bstream->isValid()), fflush(stdout);
      ...
      // NETDIAG arm: the instant the server moves us into the commander/objective/inventory view,
      // start a 60-packet full trace across the transition where the desync lives, then go quiet.
      // IMPORTANT: must NOT arm on InitialGuiMode (0) — that fires during the join handshake, and the
      // resulting printf flood across the datablock sync stalls the main thread and drops the
      // connection. Only arm on the genuine post-spawn mode changes.
      // pt.37: netDiag arming DISABLED (g_netDiag=0). In objective mode this console flood ([RPKT]/[PSTRM]/
      // [EVT]/[CMDBITS] thousands of lines/sec) saturates the browser main thread and FREEZES the client.
      // The on-demand wasm getters (wasmGetSeqDiag/EvtHist/EvtTagRaw/DnetDiag/etc) give the same info without
      // the per-packet console spam. Set g_wasmNetDiagArm=1 to re-enable for a future deep trace.
      static const int g_wasmNetDiagArm = 0;
      if(g_wasmNetDiagArm &&
         (newMode == CommandGuiMode || newMode == ObjectiveGuiMode || newMode == InventoryGuiMode)
         && g_netDiag <= 0)
      {
         g_netDiag = 12;   // tight window: keep the FIRST transition packet in the log buffer
         dbg = true;
         printf("[RPKT] *** ARM netDiag: newMode=%d (was curGuiMode=%d) entryPos=%d ***\n",
                newMode, curGuiMode, entryPos), fflush(stdout);
      }

      if(dbg) printf("[RPKT] newMode=%d curOld=%d commandTS=%p scores=%d pos=%d valid=%d\n",
                     newMode, curGuiMode, (void*)cg.commandTS, (int)scoresVisible,
                     bstream->getCurPos(), (int)bstream->isValid()), fflush(stdout);

#ifdef __EMSCRIPTEN__
      // CMDBITS RAW DUMP (strip before commit): the V0.8.5 command-mode PSC header differs from the
      // port's V0.8.1 readPacket (lastMove reads a CONSTANT 0x5C000000 only when newMode==Command2,
      // and entering command mode then disconnects). We get only ~1 command packet before the drop, so
      // capture the WHOLE PSC section raw (from the client-flag at entryPos-1, 224 bits) the instant a
      // command packet arrives — read-only (save/restore pos) — to RE the real 0.8.5 layout offline.
      // Compare against the V0.8.1 write order: guiMode(3) scores(1) lastMove(32) foFlag(1) damage(8)
      // ghostIdx(10) [controlObject::writePacketData] invFlags ... then the command sensor block.
      if(newMode == CommandGuiMode || newMode == ObjectiveGuiMode)
      {
         int save = bstream->getCurPos();
         int from = entryPos - 1;            // include the PSC present-flag PacketStream consumed
         char bits[260]; int nb = 0;
         bstream->setCurPos(from);
         for(; nb < 224 && bstream->isValid(); nb++) bits[nb] = bstream->readFlag() ? '1' : '0';
         bits[nb] = 0;
         bstream->setCurPos(save);
         // store to the global (survives console flooding); read on demand via wasmGetCmdBits.
         snprintf(g_cmdBitsBuf, sizeof(g_cmdBitsBuf), "mode=%d from=%d nb=%d firstSeq=%d raw=%s",
                  newMode, from, nb, firstMoveSeq, bits);
         // pt.37: printf DISABLED — this fired every objective-mode packet and flooded the console -> freeze.
         // g_cmdBitsBuf is still updated; read it on demand with Module.ccall('wasmGetCmdBits','string',[],[]).
      }
#endif
...
#ifdef __EMSCRIPTEN__
      wasmPscInPush(newMode, (int)lastMove, controlObject != 0);
#endif

#ifdef __EMSCRIPTEN__
      // WASM-PORT command/objective move-ack guard (KEEP): the live V0.8.5 server's command/objective-
      // mode PSC packet differs from this V0.8.1 port, so `lastMove` here reads garbage (a constant
      // ~0x5C000000). Acting on it empties our move queue and drifts firstMoveSeq AHEAD of the server's
      // lastPlayerMove; the server then computes skipCount<0 (readPacket isServer branch, ~line 782),
      // rejects EVERY packet we send, hears nothing valid from us, and times us out (the generic
      // clientDrop ~seconds after pressing O — no kick reason). The move-ack is meaningless in a non-play
      // gui mode anyway (you're observing a map, not moving an avatar), so when it is out of range here,
      // do NOT pop/advance: keep firstMoveSeq + the queue on the real play-mode base so our outbound
      // packet stays valid and the connection survives. (Valid packets always have 0 <= popMoves <=
      // moves.size(), so this never fires on a good packet; play/inventory are untouched.) The command/
      // objective view stays black until the 0.8.5 PSC layout is fully ported, but O no longer drops us.
      //
      // pt.23 FIX: fire mode-INDEPENDENTLY. During the O transition the upstream desync sometimes makes
      // PSC mis-read newMode as PlayGuiMode(1) WITH a garbage lastMove (0x5C000000) — the original guard
      // (gated on Command/Objective) missed that case, so firstMoveSeq got poisoned in "play" mode and we
      // dropped intermittently. A garbage lastMove is invalid in ANY mode; valid packets always satisfy
      // 0 <= popMoves <= moves.size(), so dropping the mode gate is safe and makes the transition survive.
      if(popMoves < 0 || popMoves > (Int32) moves.size())
      {
         if(g_netDiag > 0)
            printf("[MOVEACK] guard: newMode=%d garbage popMoves=%d (lastMove=%d firstSeq=%d) -> 0\n",
                   newMode, popMoves, (int)lastMove, firstMoveSeq), fflush(stdout);
         popMoves = 0;
      }

      // REALIGN probe (strip before commit): when lastMove is garbage, the upstream event under-read
      // its payload and shifted us. Scan forward from the PSC client-flag position for where a sane PSC
      // header (flag=1, newMode 0..6, small lastMove) realigns. The winning offset N = the number of
      // unconsumed event-payload bits = exactly what MissionResetEvent::unpack must consume. Read-only
      // (save/restore pos); runs only on an already-garbage (doomed) packet so it can't harm a good one.
      if(g_netDiag > 0 && (lastMove < 0 || lastMove > 1000000))
      {
         int flagPos = entryPos - 1;             // PacketStream read PSC's client-flag here
         int save = bstream->getCurPos();
         unsigned char buf[160]; int nb = 0;
         bstream->setCurPos(flagPos);
         for(; nb < 160 && bstream->isValid(); nb++) buf[nb] = bstream->readFlag() ? 1 : 0;
         bstream->setCurPos(save);
         for(int d = 0; d + 37 <= nb; d++)
         {
            if(buf[d] != 1) continue;             // PSC present-flag must be 1
            int nm = buf[d+1] | (buf[d+2] << 1) | (buf[d+3] << 2);
            unsigned int lm = 0;
            for(int k = 0; k < 32; k++) lm |= ((unsigned int)buf[d+5+k]) << k;   // d+4 = scoresVisible
            if(nm <= 6 && lm <= 1000000u)
            {
               printf("[REALIGN] N=%d  realigned newMode=%d lastMove=%u (firstSeq=%d) "
                      "[port mis-read newMode=%d lastMove=%d, nb=%d]\n",
                      d, nm, lm, firstMoveSeq, newMode, (int)lastMove, nb), fflush(stdout);
               break;
            }
         }
      }
      ...
      if(dbg) printf("[RPKT] A0 popMoves=%d movesz=%d lastMove=%d firstSeq=%d\n",
                     popMoves, moves.size(), lastMove, firstMoveSeq), fflush(stdout);
      // PORT HARDENING: only pop moves that actually exist. The stock loop runs `popMoves` times
      // unconditionally — when a desynced packet (e.g. after O enters CommandGuiMode) makes popMoves
      // garbage, that both pop_front()s past the end (wild access) AND advances firstMoveSeq by a
      // garbage amount, making the corruption self-sustaining. Stopping when the queue empties bounds
      // firstMoveSeq to the real move count, so the client stays sane and recovers. Does not drop the
      // packet (that broke the join handshake), so normal connect/play is unaffected.
      while(popMoves-- > 0 && moves.size() > 0)
      {
         firstMoveSeq++;
         moves.pop_front();
      }
      if(dbg) printf("[RPKT] A popMoves done (movesz=%d)\n", moves.size()), fflush(stdout);
      if(!bstream->readFlag())
         return;
      ...
      GameBase *ghost = (ghostIndex != 1023)
         ? (GameBase *) owner->getGhostManager()->resolveGhost(ghostIndex) : NULL;
      // PORT HARDENING: resolveGhost() returns NULL for an unknown/garbage ghost index (a desynced
      // packet after O enters CommandGuiMode yields a garbage ghostIndex). The stock code then sets
      // controlObject = ghost (NULL) and immediately derefs it (setPlayerControl) -> wild trap, which
      // is exactly the readPacket crash. Only do the control-object work when the ghost actually
      // resolved; a valid in-sync packet always resolves, so normal play is unaffected.
      if(ghost)
      {
...
//				camDist = 0;
      ...
      if(dbg) printf("[RPKT] B ghost done ctrlObj=%p ghostIdx-ok pos=%d valid=%d\n",
                     (void*)controlObject, bstream->getCurPos(), (int)bstream->isValid()), fflush(stdout);
      ...
      if(dbg) printf("[RPKT] C inv done pos=%d valid=%d\n",
                     bstream->getCurPos(), (int)bstream->isValid()), fflush(stdout);
      ...
      if(curGuiMode == CommandGuiMode)
      {
         ...
         // PORT HARDENING: pressing O puts a regular RPG player into CommandGuiMode (the commander /
         // objective map view) but the commander tactical display (cg.commandTS) isn't initialized
         // for non-commanders, and findClient(me) can return null. The sensor reads below MUST still
         // consume the exact bits the server wrote (or the stream desyncs), so route the per-target
         // writes through a throwaway when the real store is missing instead of dereferencing
         // cg.commandTS->targs / me — that null deref trapped in readPacket (the O-crash).
         FearGui::TSCommander::SensorTargetInfo dummy;
         if(!cg.commandTS)
            printf("[OBJFIX] CommandGuiMode packet with no commandTS — routing sensor reads to dummy\n"), fflush(stdout);
         ...
         while(bstream->readFlag())
         {
            for(int i = 0; i < 8; i++)
            {
               FearGui::TSCommander::SensorTargetInfo *info =
                  cg.commandTS ? (cg.commandTS->targs + startBlock) : &dummy;
               if(bstream->readFlag())
               {
      ...
      if(dbg) printf("[RPKT] D end pos=%d valid=%d (mode=%d)\n",
                     bstream->getCurPos(), (int)bstream->isValid(), curGuiMode), fflush(stdout);
      if(g_netDiag > 0) g_netDiag--;   // NETDIAG: wind down the post-transition capture window
   }
}
