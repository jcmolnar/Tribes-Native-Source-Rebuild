...
#include <emscripten.h>   // WASM-PORT: EMSCRIPTEN_KEEPALIVE for the chat-hide toggle export
#endif
#include "console.h"
#include <g_surfac.h>
#include <g_bitmap.h>
...
void FearGuiChatDisplay::onPreRender(void)
{
   ...
   // WASM-PORT: the IRC subsystem is deferred (IRCClient/IRCShell unported), so
   // ::IRCClient::find(manager) returned NULL and the AssertFatal guarding it compiles out
   // under NDEBUG — mIRCClient->getState() then null-derefs (OOB) the first time the in-game
   // HUD chat display prerenders after spawn. With no IRC client there's no IRC chat to pull,
   // so just skip. (In-sim PlayerSay chat comes through the game event stream, not this path.)
   if (!mIRCClient) return;
...
#ifdef __EMSCRIPTEN__
// WASM-PORT: the in-game chat/message display can be suppressed via this flag (used during debug to
// keep the green translucent fill from smearing over the 3D view / objectives page). Defaults OFF
// (chat shown) for normal play; toggle live via wasmHideChat(0/1).
bool g_wasmHideChatDisplay = false;
extern "C" EMSCRIPTEN_KEEPALIVE void wasmHideChat(int on) { g_wasmHideChatDisplay = on != 0; }
#endif
...
void FearGuiChatDisplay::onRender(GFXSurface *sfc, Point2I offset, const Box2I &updateRect)
{
   frame++;
#ifdef __EMSCRIPTEN__
   if (g_wasmHideChatDisplay) return;   // WASM-PORT: permanently suppress the chat overlay (see above)
#endif
