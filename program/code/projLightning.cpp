                             ...
                             BitStream*         pStream)
{
   ...
   if (hitting) {
      ...
      // CRASH-HARDEN: resolveGhost returns NULL for an unregistered/not-yet-received ghost index
      // (wire-read readInt(10)). AssertFatal is a no-op under NDEBUG/WASM; without a real guard the
      // dynamic_cast + deleteNotify + getTransform().p below null-deref and crash the client.
      if(!pTarget) { Net::setLastError("Invalid packet."); return; }
      m_pCurrentTarget = dynamic_cast<ShapeBase*>(pTarget);
      if(!m_pCurrentTarget) { Net::setLastError("Invalid packet."); return; }
      deleteNotify(m_pCurrentTarget);
   ...
   if (stream->readFlag() == true) {
      ...
      if (gottaTarget == true) {
         ...
         if (hasTarget) {
            ...
            SimNetObject* pTarget = gm->resolveGhost(ghostIdNew);
            // CRASH-HARDEN: resolveGhost returns NULL for an unregistered/not-yet-received ghost
            // index (wire-read readInt(10)). AssertFatal is a no-op under NDEBUG/WASM; without a
            // real guard deleteNotify(newTargetPtr) null-derefs and crashes the client.
            if(!pTarget) { Net::setLastError("Invalid packet."); return; }

            ShapeBase* newTargetPtr = dynamic_cast<ShapeBase*>(pTarget);
            if(!newTargetPtr) { Net::setLastError("Invalid packet."); return; }
