...
bool FearCSDelegate::simConnect(const char *transportAddress)
{
   ...
   // WASM-PORT: the live "Last Hope" Kronos server (0.8.5 / build 1.30) gates connects on the
   // version string "1.30" — proven by the decode harness (test/enginepstream.cpp buildFearConnect).
   // The 0.8.1 source here wrote a stale "1.11" (its FearVersion-munging above produced "1.0", hence
   // the hardcode + TODO), which the live server rejects with "Wrong game version." Send "1.30".
   bstream.writeString("1.30");
...
void FearCSDelegate::gotGameInfo(const GameInfo &gameInfo)
{
   ...
   // WASM-PORT: the wire packs player counts as a single BYTE each (server-side getGameInfo
   // writes BYTE(numClients)/BYTE(maxPlayers); confirmed against the live server by the proven
   // enginequery decode). This 0.8.1 reconstruction read them as int (4 bytes), which desynced
   // the stream — corrupting the counts AND every field after (mission name, mods, ...). Read a
   // BYTE into a temp, then widen into the int members.
   BYTE numPlayersByte;
   bstream.read(&numPlayersByte);
   newServer->numPlayers = numPlayersByte;

   //get the maximum number of players
   BYTE maxPlayersByte;
   bstream.read(&maxPlayersByte);
   newServer->maxPlayers = maxPlayersByte;
