...
#include <limits.h>     // WASM-PORT: INT_MAX (MSVC pulled it via a forced include)
#include <sim.h>
#include <ml.h>
#include <ts.h>
...
void TSCommander::buildMap()
{
   ...
   // WASM-PORT: the commander overhead map is built by rendering the whole world
   // (terrain + Sky + objects) top-down into 128x128 GFXBitmap tiles through a
   // software GFXMemSurface, then memcpy'ing the rasterized pixels back. That path
   // is the CPU scanline rasterizer (DeltaCalcPolyW / GFXRenderSpan*), which is
   // stubbed-to-trap in the GL-only browser build. Before lushWorld.vol was mounted
   // this happened to early-out (no SimPalette -> pal invalid -> return at the !pal
   // check below); now the palette loads and it would reach the software render and
   // abort during the spawn handshake (onGhostAlwaysDone -> c_rebuildCommandMap).
   // Skip it: the overhead command map stays blank, but the player spawns and the
   // 3D play view (TSControl::onRender -> the GL canvas surface) renders normally.
   return;
#endif
   zoomLevel = 0;
   zoomMode = FALSE;
