...
// WASM-PORT: free-function twin of FGHPaletteTable::InitializeTable. Callers
// (FHinventory/HudManager onRender) have no table instance, so source the rgb
// from the gPaletteColors table itself rather than a gHudColors member.
void InitializeHudColors(GFXPalette *pal)
{
   for (int i = 0; i < _NUM_HUD_COLORS; i++)
   {
