...
void TeamObjectiveEvent::unpack(SimManager *manager, Net::PacketStream *ps, BitStream *bs)
{
   ...
   // WASM-PORT: DATA-CONFIRMED live format (full-packet capture re/pktcap_objburst.txt, all 26 lines decoded):
   // the objective event = [objNum:32-bit LSB][writeString(text)] — IDENTICAL to the version-matched format below.
   // The scan-heuristic + g_objTrailBits is GONE: it was compensating for the wrong TAG, not a wrong format. The
   // real bug was TeamObjectiveEventType=1024 / MissReset=1113 in FearDcl.h (the live tag is 1113); create(1113)
   // ran MissReset's 0-bit unpack and dropped every objective, and the dropped objNum's low bits read as a
   // phantom tag-1024 event. With the tag fixed (FearDcl.h: TeamObjective=1113), this simple read decodes every
   // objective in order; objNum IS the real line number (1..26), so slot by it directly (handler does
   // case TeamObjectiveEventType -> slot = evt->objNum). Empty lines (e.g. 19) arrive as objNum + empty string.
   bs->read(&objNum);
   bs->readString(text);
   address.objectId = PlayerManagerId;
#else
   // version-matched / 0.8.1 standard format: write(objNum)[32-bit] + writeString(text). objNum is the
   // real line number (no divergence vs a Tribes.exe built from the same TribesSource).
   bs->read(&objNum);
   bs->readString(text);
   address.objectId = PlayerManagerId;
#endif
...
// WASM-PORT netcode-vs-render probe (strip before commit): dump the CLIENT's stored objective text
// (cg.playerManager->teamList[*].objectives[*].text). If non-empty after pressing O => the text DID
// arrive over the wire and the blank center is a GUI-RENDER gap (FGHCommandObjectivePage); if empty =>
// the TeamObjectiveEvent never decoded (netcode). Read via Module.ccall('wasmGetObjectives','string',[],[]).
extern "C" EMSCRIPTEN_KEEPALIVE const char* wasmGetObjectives() {
   static char buf[2048]; int w = 0; buf[0] = 0;
   PlayerManager* pm = cg.playerManager;
   if(!pm) { snprintf(buf, sizeof(buf), "no cg.playerManager"); return buf; }
   for(int t = 0; t < (int)PlayerManager::MaxTeams && w < (int)sizeof(buf) - 80; t++)
      for(int j = 0; j < (int)MAX_NUM_OBJECTIVES && w < (int)sizeof(buf) - 80; j++) {
         const char* tx = pm->teamList[t].objectives[j].text;
         if(tx[0]) w += snprintf(buf + w, sizeof(buf) - w, "[t%d/%d]%.36s | ", t, j, tx);
      }
   if(w == 0) snprintf(buf, sizeof(buf), "(all objectives empty)");
   return buf;
}
#endif
...
public:
   ...
   static void post(SimManager *manager, int destPlayerId, int commanderId, int targetId, int command, const char *message, int ctarget, const Point2I &wayPoint, int status, bool statusOnly);
...
void PlayerCommandEvent::post(SimManager *manager, int destPlayerId, int commanderId, int targetId, int commandId, const char *message, int ctarget, const Point2I &wayPoint, int status, bool statusOnly)
{
...
{
   // NATIVE-PORT: never re-insert a node already in the list. Re-adding it creates a CYCLE (its old
   // predecessor still points to it while it now points to the new head), and the many clientLink/
   // baseRepLink traversals (e.g. PlayerTeamChange, PlayerManager.cpp:2053) then infinite-loop ->
   // the client HANGS (observed live after a while in a populated server). Walk is self-capped so a
   // pre-existing cycle can't hang the guard itself.
   { int _g = 0;
     for(PlayerManager::ClientRep* c = dynamic_cast<PlayerManager::ClientRep*>(*listPtr);
         c && _g < 8192; c = c->nextClient, ++_g)
        if(c == dynamic_cast<PlayerManager::ClientRep*>(elemPtr)) return; }
...
void PlayerManager::issueCommand(int cmdrId, int targId, int commandIcon, const char *commandString, const Point2I &wayPoint, int objectIndex)
{
   if (! isServer) return;
   ...
   switch (event->type)
   {
      ...
      case TeamObjectiveEventType:
         if (! isServer)
         {
            ...
            int slot = evt->objNum;
#ifdef __EMSCRIPTEN__
            // WASM-PORT pt.40: objNum is the REAL line number (standard tag-1151 format, version-matched
            // server). Slot by it directly, bounds-checked (drop a stray out-of-range index rather than
            // scribbling teamList). MAX_NUM_OBJECTIVES=32; the RPG numbers lines 1..26.
            if(!me) break;
            if(slot < 0 || slot >= (int)MAX_NUM_OBJECTIVES) break;
#endif
            strcpy(teamList[me->team+1].objectives[slot].text, evt->text);

#ifdef __EMSCRIPTEN__
            // WASM-PORT: the "CmdObjectivesGui" named-object lookup below never resolves in our shell
            // (that name came from a script-loaded .gui we don't run), so the page never got populated.
            // Populate the live page directly via the pointer it stashes in onAdd. This also closes the
            // TIMING gap: the page's onWake reads teamList BEFORE this event arrives over the wire, so
            // without this the page is sized to height 0. Force it visible too (it's authored vis=0 and
            // nothing in our port shows it). newObjective() formats the text and resizes the page to fit.
            // extent.x>0 guard: formatControlString into a zero width can spin (the old findObject path
            // guarded this too). When unsized, the line still lands in teamList above, and the page's
            // onWake pulls it from teamList (after setting extent.x) the next time O opens.
            if (FearGui::g_wasmCmdObjPage)
            {
               // WASM-PORT: re-pull the WHOLE teamList (not just this one slot) so a line landing after the
               // page already woke shows. refreshFromTeamList self-guards width, so no extent.x>0 gate needed.
               FearGui::g_wasmCmdObjPage->refreshFromTeamList();
               FearGui::g_wasmCmdObjPage->setVisible(true);
               FearGui::g_wasmCmdObjPage->setUpdate();
            }
#endif
...
#ifdef __EMSCRIPTEN__
               // only format/render when the page is actually sized (extent.x>0) — formatControlString into a
               // zero width can spin; and the validity gate above already dropped garbage.
               if (objCtrl && objCtrl->extent.x > 0) objCtrl->newObjective(slot, evt->text);
#else
               if (objCtrl) objCtrl->newObjective(slot, evt->text);
#endif
            }
         }
         break;
