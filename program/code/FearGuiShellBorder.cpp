...
void FGShellBorder::onRender(GFXSurface* sfc, Point2I offset, const Box2I& updateRect)
{
   ...
   // WASM-PORT: the border chrome comes from the bitmap array named by the SimTagDictionary
   // (IDPBA_SHELL_BORDER -> "ShellBorderCtrl.PBA"), populated by fear.strings.cs. If that
   // tagged-string script wasn't exec'd the tag is unmapped, loadBitmapArray leaves bma
   // invalid, and GFXBitmapArray::getBitmap (array[index], no bounds check) would OOB. The
   // original caught this with AssertFatal, which compiles out under NDEBUG -> latent trap.
   if (! (bool)bma) return;
...
void FGShellBorder::loadBitmapArray(void)
{
   ...
   // WASM-PORT: under NDEBUG AssertFatal is a no-op, so guard explicitly — an unmapped tag
   // (fear.strings.cs not exec'd) leaves bma invalid; onRender bails on that. See onRender.
   if (! pbaName) return;
