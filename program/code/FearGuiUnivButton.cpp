...
// WASM-PORT: was `inline` — an inline definition in a .cpp emits no external symbol, so
// other TUs that construct FGUniversalButton (FearGuiRadio/Switch/etc.) got an undefined
// reference at link. Drop inline so the ctor links normally.
FGUniversalButton::FGUniversalButton()
{
   active = true;
   mbSwitch = false;
   mbSet = false;
