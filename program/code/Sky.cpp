...
void Sky::inspectRead(Inspect *inspect)
{
   ...
   inspect->read(IDITG_SKY_ROTATION, featurePosition);
   inspect->read(IDITG_SIZE,   size);
...
void Sky::render(TSRenderContext &rc)
{
   ...
   // WASM-PORT: keep a clean flat sky fill. Enabling the textured band path (renderSkyBand,
   // lushsky_night.dml) rendered the sky solid WHITE in the GL-emulation — the band textures don't
   // bind/sample (a missing-texture default sampled white, then the fullbright textured path
   // modulates by white). After terrain mounted, Sky::initialize() ALSO re-sampled skyColor/skyIndex
   // from those (white) bitmaps, so even the solid fill could read white. Force a fixed sky colour
   // (skyIndex=-1 -> setFillColor finds the nearest palette entry to skyColor each frame) so it's
   // deterministic and never white. The real textured night sky needs in-browser debugging of the
   // band texture upload (likely a DML/BMP name mismatch: BMPs are "lushksky_night*.bmp" with a 'k').
   skyColor = ColorF(0.40f, 0.52f, 0.74f);   // clean sky blue (placeholder; tune in-browser later)
   skyIndex = -1;
   renderSolid(rc);
   return;
#endif
