...
void DataBlockEvent::unpack(SimManager *manager, Net::PacketStream *ps, BitStream *bstream)
{
   ...
   if(block != 255)
   {
      ...
      // CRASH-HARDEN: createDataBlock returns NULL for any group with no switch case
      // (group is a wire-read readInt(6) = 0..63; valid groups < NumDataTypes). A malformed
      // packet — or any upstream desync that garbles `group` — would NULL-deref here. Treat as
      // an invalid packet instead of crashing the client against an untrusted live server.
      if(!data) { Net::setLastError("Invalid packet."); return; }
      data->unpack(bstream);
   }
   else
      block = -1;
...
// WASM-PORT: one-shot guard so the data-finished handshake (see processEvent FINAL block) fires
// exactly once per connection. Reset in reset(), which onConnectionAccepted calls per new session.
static bool gWasmDataFinishedSent = false;
#endif
...
void DataBlockManager::reset()
{
   ...
   curGroup = -1;
#ifdef __EMSCRIPTEN__
   gWasmDataFinishedSent = false;   // WASM-PORT: re-arm the data-finished handshake for a new sync
#endif
...
bool DataBlockManager::processEvent(const SimEvent *event)
{
...
#ifdef __EMSCRIPTEN__
   // WASM-PORT (TEMP DEBUG): trace datablock sync progress. The whole spawn handshake is gated on
   // this reaching the final block (line ~318 -> dataFinished). Log each new group + the final.
   {
      static int s_lastGroup = -2, s_count = 0;
      s_count++;
      if(evt->group != s_lastGroup) {
         printf("[WASM-PORT][DBM] group=%d (\"%s\") gsize=%d block=%d  NumDataTypes=%d  totalEvents=%d\n",
                evt->group, (evt->group>=0?dataNames[evt->group]:"?"), evt->gsize, evt->block,
                (int)NumDataTypes, s_count);
         fflush(stdout);
         s_lastGroup = evt->group;
      }
   }
#endif
   ...
   if(evt->block != -1)
   {
      ...
      {
#ifdef __EMSCRIPTEN__
         // WASM-PORT (skip-and-continue): a missing/new-art shape ("Unable to model: X.dts") used to
         // Net::setLastError -> DISCONNECT, which blocks load-in on any server carrying art we don't
         // have provisioned. Log and SKIP the block instead, falling through so the remaining
         // datablocks still load and we can spawn — the missing shape simply won't render. (The block
         // is already stored above; its shape resource is just NULL.) Native keeps the strict behavior.
         printf("[WASM-PORT][DBM] preload FAILED (SKIPPED) group=%d (\"%s\") block=%d/%d: %s\n",
                evt->group, (evt->group>=0?dataNames[evt->group]:"?"), evt->block, evt->gsize-1,
                errorBuffer);
         fflush(stdout);
#else
   ...
   {
#ifdef __EMSCRIPTEN__
      // WASM-PORT: drive the data-finished handshake DIRECTLY, once per sync. The mod's client
      // dataFinished() (Client.cs) is GUI-coupled — it sets ProgressText/ProgressSlider on the
      // Loading.gui that the headless harness never builds; that Control::setValue on a missing
      // control aborts the function BEFORE its `remoteEval(2048, dataFinished)`, so $dataFinished
      // never sets and the server keeps re-sending the final datablock forever (never advancing to
      // ghosting -> the VC eventually times out and drops). So set the flag + send the remoteEval
      // ourselves. The guard also stops the server's per-tick re-sends from re-firing this.
      if(gWasmDataFinishedSent)
         return true;
      gWasmDataFinishedSent = true;
      printf("[WASM-PORT][DBM] FINAL block reached (group=%d block=%d) -> driving dataFinished handshake\n",
             evt->group, evt->block);
      fflush(stdout);
      Console->evaluate("$dataFinished = true; remoteEval(2048, dataFinished);", false);
#endif
