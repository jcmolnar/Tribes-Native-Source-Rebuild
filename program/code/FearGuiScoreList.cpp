...
void ScoreListCtrl::onPreRender()
{
   ...
   // PORT HARDENING: tabStops are fixed MaxColumns arrays; clamp stop so a heading with more columns
   // than that can't overflow on write (the terminator slot must stay in bounds too).
   BYTE *s = (BYTE *) playerManager->teamScoreHeading;
   while(*s)
   {
      if(*s == '\t' && *(s+1) && stop < MaxColumns - 1)
      {
...
void ScoreListCtrl::onRenderCell(GFXSurface *sfc, Point2I offset, Point2I cell, bool, bool)
{
   ...
   if(cell.y >= firstClientLine)
   {
      ...
      int pidx = cell.y - firstClientLine;
      // PORT HARDENING: the ArrayCtrl cell count (sized from getNumClients) can outrun the players
      // vector (walked from getClientList) during the score-screen teardown / observe-client frame,
      // and a ClientRep's scoreString may be null before the server fills it. Either deref is then a
      // wild pointer -> wasm "memory access out of bounds" trap (reproduced closing the score screen).
      // Guard the indices/pointers and render the cell empty instead of crashing.
      const char *s = (pidx >= 0 && pidx < players.size() && players[pidx] && players[pidx]->scoreString)
                      ? players[pidx]->scoreString : 0;
      if(s && players[pidx]->id == selectedClient)
         selected = true;
      char *bufptr = buf;
   ...
   else if (cell.y >= firstTeamLine)
   {
      ...
      // PORT HARDENING: same guard as the client branch — teams vector / scoreString can be short or
      // null during teardown; a wild deref here traps. Render empty instead of crashing.
      const char *s = (team >= 0 && team < teams.size() && teams[team] && teams[team]->scoreString)
                      ? teams[team]->scoreString : 0;
      char *bufptr = buf;
   ...
   for(;;)
   {
      if(stop + 1 >= MaxColumns)   // PORT HARDENING: a scoreString with more \t fields than the
         break;                    // 16-slot tabStops array would read past its end — bound it.
