...
void Player::ghostSetMove(PlayerMove *move, Point3F &newPos, Point3F &newVel, bool cont, float newRot, float newPitch, int skipCount, bool noInterp)
{
   ...
   // WASM-PORT: this observe/decode client never ticks ghosted players' clientProcess, so
   // updateAnimation never drives the vertical-aim "looks" blend (it stays at its init seed; see the
   // SetSequence 0.5 level-seed in player.cpp initResources). Drive it HERE from the freshly-decoded
   // network pitch so observed players visibly aim up/down (PvP read of where others are looking).
   // newPitch is the network target in radians [-MaxPitch,+MaxPitch]; the blend is 0=up .5=level 1=down.
   if (viewThread)
   {
      viewThread->SetSequence(crouching ? crouchLooksSequence : looksSequence);
      float lpos = 0.5f + (-newPitch / MaxPitch) * 0.5f;
      if (lpos < 0.0f) lpos = 0.0f; else if (lpos > 1.0f) lpos = 1.0f;
      viewThread->SetPosition(lpos);
   }
#endif
}
...
#ifdef __EMSCRIPTEN__
// WASM-PORT: public wrapper so the observe client's onIdle can drive a (ghosted) player's protected
// clientProcess each frame — giving position interpolation (stepDelta) between network packets (smooth
// movement) AND body/arm-fire/weapon-image animation. The same-frame guard mirrors main.cpp:384 so we
// don't double-tick. clientProcess's decal/footstep block is now dmgr-null-guarded, so it's safe here.
void Player::wasmTickClient(DWORD curTime)
{
   if (getLastProcessTime() != curTime)
      clientProcess(curTime);
}
#endif
...
void Player::clientProcess(DWORD curTime)
{
   ...
   // WASM-PORT: guard on dmgr — the observe/decode client may have no DecalManager installed, in
	// which case dmgr is NULL (the AssertFatal above is a no-op under NDEBUG). The footstep decals
	// below would then null-deref. Skipping the whole block (incl. foot sounds) is harmless for the
	// observe client and lets clientProcess be ticked for ghosts to get position interpolation.
   if (dmgr && surfaceType != -1 && !mount && lastContactCount < 8)
	{
	   int trigger[1];
...
void Player::unpackUpdate(Net::GhostManager *gm, BitStream *stream)
{
   ...
   if(stream->readFlag())
   {
      ...
      if(stream->readFlag())
      {
         ...
         // CRASH-HARDEN: resolveGhost returns NULL if the server sends a ghost index that hasn't
         // been received on the client yet (wire value from readInt(10); can be any 0..1023).
         // AssertFatal is a no-op under NDEBUG/WASM; without a real guard, deleteNotify(newMount)
         // below null-derefs. Treat as invalid packet to keep the session alive rather than crash.
         if(!newMount) { Net::setLastError("Invalid packet."); return; }
