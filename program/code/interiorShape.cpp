...
#include "wasm_logonce.h"
#endif
#ifdef __EMSCRIPTEN__
#include <emscripten.h>     // WASM-PORT diag: record every interior's real m_pFileName into window.__interiors
#endif
#include <itrShape.h>
   ...
   m_pFileName(NULL)
{
   ...
   // WASM-PORT: the ctor never initialized m_renderImage.currShape, so getInstance() returned
   // UNINITIALIZED GARBAGE. onAdd()'s first line is set(rot,pos) -> setPosition -> buildContainerBox,
   // which runs BEFORE initInteriorResources() creates the instance, so the null guards there
   // (getInstance()==NULL) saw garbage and the getGeometry() deref read out of bounds. This only
   // surfaced under wasm's heap layout (live Kronos ghost stream adding world interiors). NULL it.
                                    ...
                                    Box3F*        out_pBox)
{
   ...
   // WASM-PORT: a ghosted world interior whose geometry sub-resource didn't load (the observe
   // client skips most mission interior vols) has a NON-NULL instance but a NULL current
   // geometry, so getGeometry()->box reads out of bounds. Bail like the null-instance guard —
   // an unrendered interior simply contributes no container box. (Trap surfaced live during the
   // Kronos ghost stream: onReceive -> GhostManager::readPacket -> InteriorShape::onAdd here.)
   if (getInstance() == NULL || getInstance()->getGeometry() == NULL)
      return;
...
InteriorShape::initRenderImage()
{
   ...
   // WASM-PORT: SimRenderImage's ctor never initializes itype (simRenderGrp.h:32 sets only
   // visibility+hazeValue), so the interior's render image had a GARBAGE itype. The container
   // render switch (simContainer.cpp:823) only routes Normal/Translucent/Overlay/Background to a
   // render list — a garbage itype matched NO case, so the interior image was silently DROPPED and
   // ITRRender::render was never called (the whole interior went unrendered). The 32-bit build got
   // away with zero-inited memory (==Normal). Set it explicitly, exactly as SimTerrain does.
   m_renderImage.itype = SimRenderImage::Normal;
...
//
#ifdef __EMSCRIPTEN__
// WASM-PORT on-demand interior streaming. The server ghosts every mission interior to us, each
// carrying its filename (m_pFileName). Instead of a hand-curated allow-list + baked vols, onAdd tries
// to load immediately; if the containing vol isn't mounted yet, the ghost is QUEUED (by SimObjectId,
// so a scoped-out ghost can't leave a dangling pointer) and JS is asked to fetch+mount the vol
// (asset-server /find -> streamVol). wasmDrainInteriors() then retry-builds the arrived ones a few per
// frame — which also retires the synchronous-mass-build hang the old allow-list existed to avoid.
static Vector<SimObjectId> g_wasmPendingInteriors;   // engine Vector (libc++ <vector> pulls nanosleep)
static SimManager*         g_wasmInteriorMgr = NULL;

static void wasmQueueInterior(SimManager* mgr, SimObjectId id, const char* file)
{
   g_wasmInteriorMgr = mgr;
   for (int i = 0; i < g_wasmPendingInteriors.size(); i++)
      if (g_wasmPendingInteriors[i] == id) return;            // already queued
   g_wasmPendingInteriors.push_back(id);
   EM_ASM({ if (Module.streamAsset) Module.streamAsset(UTF8ToString($0)); }, file);
}

// Build up to maxBuilds queued interiors whose vol has now arrived; call once per frame from the main
// loop. findObject(id)==NULL => the ghost scoped out (drop it); getInstance()!=NULL => already built
// (drop it); initInteriorResources() fails fast (cheap findFile) while the vol is still missing (keep
// waiting). Only successful BUILDS count against maxBuilds — that's the heavy decompress+BSP+lighting.
extern "C" EMSCRIPTEN_KEEPALIVE int wasmDrainInteriors(int maxBuilds)
{
   if (g_wasmInteriorMgr == NULL) return 0;
   int built = 0;
   for (int i = 0; i < g_wasmPendingInteriors.size() && built < maxBuilds; )
   {
      SimObject*     o  = g_wasmInteriorMgr->findObject(g_wasmPendingInteriors[i]);
      InteriorShape* is = dynamic_cast<InteriorShape*>(o);
      if (is == NULL || is->getInstance() != NULL) {          // gone, or already built elsewhere
         g_wasmPendingInteriors.erase(i);
         continue;
      }
      if (is->initInteriorResources()) {                      // vol arrived + geometry built
         is->remapTeamTextures(is->getTeam());                // load the textured material list — this is
         is->initLightParams();                               // the texture half of onAdd's setTeam() path
         is->restartLights();                                 // (setTeam is protected; remap is public).
         built++;
         g_wasmPendingInteriors.erase(i);
      } else {
         i++;                                                 // vol not mounted yet — keep waiting
      }
   }
   return built;
}
...
InteriorShape::onAdd()
{
   ...
   shutdownLights();
#ifdef __EMSCRIPTEN__
   // WASM-PORT: demand-stream interiors (replaces the old static allow-list). Try to load now — the vol
   // is mounted only for the spawn building(s) baked in, so most ghosts fail fast here (a cheap findFile
   // miss, NOT a build, so no hang) and get queued: wasmQueueInterior asks JS to fetch+mount the vol
   // that holds this interior (asset-server /find), and wasmDrainInteriors (per-frame, throttled)
   // retry-builds it once the vol arrives. The whole mission's buildings stream in keyed off the names
   // the server already ghosts to us — no allow-list, no hand-curated mount set.
   bool fInit = initInteriorResources();
   if (fInit == false && isGhost() == true && m_pFileName && m_pFileName[0] != '\0')
      wasmQueueInterior(manager, getId(), m_pFileName);
   // WASM-PORT diag: record EVERY interior's real filename + load result (read via window.__interiors).
   EM_ASM({
      if (!window.__interiors) window.__interiors = [];
      window.__interiors.push(UTF8ToString($0) + "  fInit=" + $1 + " currShape=" + $2);
   }, m_pFileName ? m_pFileName : "(null)", (int)fInit, (int)(m_renderImage.currShape != NULL));
#else
   bool fInit = initInteriorResources();
#endif
   ...
   if(isGhost() == true) {
      if (fInit == false) {
#ifdef __EMSCRIPTEN__
         // WASM-PORT: the observe/decode client doesn't carry the mission's custom buildings
         // (the .dis interiors live in the per-mission interior vols we skip mounting). The base
         // client disconnects here; instead log once and KEEP the ghost (no instance — the
         // buildContainerBox/light paths are null-guarded), so the session holds and the rest of
         // the ghost stream keeps decoding. A real rendering client must preload these buildings.
         if (wasmLogOnce(m_pFileName))
            printf("[WASM-PORT] InteriorShape: building \"%s\" not loaded (session continues; "
                   "a real client must preload it)\n", m_pFileName);
#else
...
#ifdef __EMSCRIPTEN__
         // WASM-PORT: our local building .dis may be a slightly different version than the live
         // server's (dev tree V0.8.1 vs live V0.8.5), so the per-building checksum won't match.
         // The base client REJECTS the connection here (this is exactly the keep.0.dis break that
         // KingdomKronos.vol caused — see fa35c82). For the observe/decode client we'd rather see
         // the building (even a near-version) than drop the session: log once and RENDER what we
         // loaded instead of rejecting. The geometry is already built (fInit==true).
         if (wasmLogOnce(m_pFileName))
            printf("[WASM-PORT] InteriorShape: building \"%s\" CRC mismatch (rendering local "
                   "version anyway; live server has a different build)\n", m_pFileName);
#else
...
InteriorShape::render(SimContainerRenderContext& rc)
{
   if (m_renderImage.currShape == NULL)
      return false;
#ifdef __EMSCRIPTEN__
   // WASM-PORT diag: count calls to a LOADED interior's container render (currShape!=NULL) and whether
   // we reach the bottom (rc.render(this) -> image render). __isr vs __isrReached tells us if the early
   // returns below (linked/contextTag) bail; __rimg (in RenderImage::render) tells us if rc.render(this)
   // actually invokes the image render that calls ITRRender.
   EM_ASM({ window.__isr = (window.__isr||0) + 1; });
#endif
...
#ifdef __EMSCRIPTEN__
   EM_ASM({ window.__isrPast = (window.__isrPast||0) + 1; });  // WASM-PORT diag: passed linked/contextTag guards
#endif
...
{
#ifdef __EMSCRIPTEN__
   EM_ASM({ window.__rimg = (window.__rimg||0) + 1; });  // WASM-PORT diag: image render reached -> ITRRender next
#endif
